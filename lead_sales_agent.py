...
class LeadSalesAgent:
    ...
    """
    async def execute_sales_burst(self, niche="Missouri"):
        colony_log(f"SALES: Initiating REAL-WORLD liquidation burst for [{niche}] leads...", node="SUPREME")
        ...
        if res.get("status") == "success":
            colony_log(f"✓ SALES SUCCESS: Invoice for ${amount} published at 'willow rain Co'.", node="SUPREME")
            db.log_event("SALES", "INVOICE_PUBLISHED", {"client": client_name, "amount": amount, "invoice_id": res['invoice_id']})
        else:
            colony_log(f"[-] SALES ERROR: {res.get('message')}", node="SUPREME")
