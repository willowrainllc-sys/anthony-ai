...
class PortReputationGuardian:
    ...
    async def audit_matrix_reputation(self) -> dict:
        colony_log("REPUTATION_GUARD: Initiating round-trip proxy reputation audit...", node="REP_GUARD")
        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=10.0) as client:
                ...
                if resp.status_code == 200:
                    ...
                    colony_log(f" REP_GUARD SUCCESS: Matrix Reputation Score: {reputation_score}/100. Target: [{org}]", node="REP_GUARD")
                    db.log_event("REP_GUARD", "REPUTATION_AUDIT_COMPLETE", summary)
                    return summary

        except Exception as e:
            colony_log(f"[-] REP_GUARD Error: {e}", node="REP_GUARD")
