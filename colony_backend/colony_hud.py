...
DB_PATH = Path(r"D:\ObsidianAi_Colony\Empire_Vault.db")
LOG_PATH = Path(r"D:\ObsidianAi_Colony\Logs\empire_master.log")
...
def get_vitals():
    try:
        ...
        # 2. Task Stats
        c.execute("SELECT status, COUNT(*) FROM colony_tasks GROUP BY status")
        ...
        c.execute("SELECT node, timestamp, metadata FROM empire_events WHERE event_type='BURST_SUCCESS' ORDER BY id DESC LIMIT 3")
        ...
        for row_m in raw_missions:
            ...
            social_missions.append({
                "title": p.get('title', 'Intel Burst'),
                "channel": row_m[1],
                "priority": row_m[2],
                "created_at": row_m[3]
            })

        # 7. Burst Clocks (Filtered to exclude internal video_use)
        c.execute("SELECT channel, last_burst, total_bursts FROM channel_clocks WHERE channel NOT IN ('VIDEO_USE', 'QUANTUM_LOCK')")
        clocks = c.fetchall()
...
def run_hud():
    ...
    while True:
        try:
            ...
            print(f"{Fore.GREEN}[SYS_VITALS]{Style.RESET_ALL} | {Fore.YELLOW}Negotiating: {negotiating_count}{Style.RESET_ALL} | {Fore.GREEN}Active: {processing_count}{Style.RESET_ALL} | {Fore.GREEN}Bursts: {completed_count}{Style.RESET_ALL} | {Fore.RED}Failed: {failed_count}{Style.RESET_ALL}")
            ...
            if not recent_drops:
                ...
                for node, ts, meta_str in recent_drops:
                    ...
                    try:
                        ...
                        title = meta.get('title', 'Obsidian Burst')
                    except:
                        title = "Obsidian Burst"
                    print(f"  {Fore.GREEN} [{drop_time}]{Style.RESET_ALL} {Fore.YELLOW}{node}:{Style.RESET_ALL} {Fore.WHITE}{title[:45]}{Style.RESET_ALL}")

            # --- SECTION 7: BURST CLOCKS ---
            print(f"\n{Fore.YELLOW}[BURST_CLOCKS // PACING_MATRIX]{Style.RESET_ALL}")
            if not clocks:
