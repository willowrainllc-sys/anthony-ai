...
class ObsidianOSBootloader:
    ...
    async def execute_boot_sequence(self):
        ...
        env = os.environ.copy()
        env["PYTHONPATH"] = str(self.root_dir) + os.pathsep + str(self.root_dir / "colony_backend")

        subprocess.Popen(f"python {self.root_dir}/colony_backend/anthony_command_os.py", shell=True, env=env)
        await asyncio.sleep(5)

        # 2. Start Network Fabric
        colony_log("[*] BOOT: Igniting 5G Fiber Mesh & VDC Hypervisor...", node="SUPREME")
        ...
        colony_log("[*] BOOT: Registering 103 Humanoid ASI Nodes (Autonomous)...", node="SUPREME")
        subprocess.Popen(f"python {self.root_dir}/willow_rain_global/cellular_stack/aiphony_provisioner.py", shell=True, env=env)
        subprocess.Popen(f"python {self.root_dir}/colony_backend/obsidian_hive_heartbeat.py", shell=True, env=env)
        ...
        colony_log("[*] BOOT: Igniting Sovereign VPN (Self-Made) & Mesh...", node="SUPREME")
        subprocess.Popen(f"python {self.root_dir}/colony_backend/obsidian_sovereign_vpn.py", shell=True, env=env)
        subprocess.Popen(f"python {self.root_dir}/colony_backend/obsidian_wifi_engine.py", shell=True, env=env)
        subprocess.Popen(f"python {self.root_dir}/colony_backend/obsidian_ble_mesh.py", shell=True, env=env)
