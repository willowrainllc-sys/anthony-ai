...
async def execute_moderna_burst():
    ...
    colony_log(f"[SUPREME] TEST BURST: Moderna High-Fidelity mission for {TARGET_VIDEO.name}...", node="MOBILE")
    ...
    if switcher:
        colony_log("MANUAL: Switching to Willow Rain profile...", node="MOBILE")
    ...
    if media_btn:
        ...
        # 7. FINAL DISPATCH
        agent.tap(650, 100)
        colony_log("SUCCESS: Moderna High-Fidelity Burst Dispatched.", node="MOBILE")
...
if __name__ == "__main__":
    asyncio.run(execute_moderna_burst())
