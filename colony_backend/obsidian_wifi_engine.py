...
class ObsidianWifiEngine:
    ...
    2. CAPTIVE PORTAL: Intercepts connection requests and forces the Obsidian Connection.
    3. DNS REDIRECTION: Resolves all traffic through the private Missouri Matrix.
    4. MESH BROADCAST: Projects the 'Obsidian_Sovereign' SSID to the Nest.
    """
    def __init__(self):
    ...
    async def ignite_hotspot(self):
        colony_log(f"📡 WIFI: Igniting Sovereign Hotspot [{self.ssid}]...", node="NETWORK")
        ...
        try:
            ...
            self.is_active = True
            colony_log("[+] WIFI SUCCESS: Hotspot is LIVE. Nest nodes are handshaking.", node="NETWORK")
