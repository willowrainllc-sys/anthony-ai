...
class ObsidianPayKernel:
    ...
    def create_account(self, cashtag, user_id):
        ...
        with db._get_connection() as conn:
            ...
            conn.commit()
        colony_log(f"PAY: New account created -> ${cashtag}", node="FINANCE")
        return f"${cashtag}"
    ...
    def transfer(self, sender, receiver, amount, note=""):
        ...
        with db._get_connection() as conn:
            ...
            conn.commit()
            colony_log(f"PAY: Industrial Transfer SUCCESS. {sender} -> {receiver} [Net: ${net_amount}, Fee: ${fee}]", node="FINANCE")
            return True, tx_id
