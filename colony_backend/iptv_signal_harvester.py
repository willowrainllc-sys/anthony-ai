...
class IptvSignalHarvester:
    ...
    async def refresh_signal_matrix(self) -> int:
        ...
        colony_log("IPTV_HARVESTER: Refreshing global signal matrix...", node="IPTV_NODE")
        ...
        colony_log(f" IPTV_HARVESTER SUCCESS: Matrix refreshed. {len(new_streams)} live signals active.", node="IPTV_NODE")
    ...
    async def clip_media_signal(self, stream_id: str, duration_sec: int = 15) -> Optional[str]:
        ...
        if not stream:
            colony_log(f"[-] IPTV_CLIPPER: Stream [{stream_id}] not found.", node="IPTV_CLIPPER")
            return None

        colony_log(f"IPTV_CLIPPER: Clipping {duration_sec}s segment from [{stream.name}]...", node="IPTV_CLIPPER")

        output_filename = f"clip_{stream_id}_{uuid.uuid4().hex[:6]}.mp4"
        output_path = Path(r"D:\ObsidianAi_Colony\Secure_Assets\source_videos") / output_filename
        ...
        try:
            ...
            if output_path.exists() and output_path.stat().st_size > 1000:
                colony_log(f" IPTV_CLIPPER SUCCESS: Saved media clip to {output_path.name}", node="IPTV_CLIPPER")
                return str(output_path)
            else:
                colony_log(f"[-] IPTV_CLIPPER: Clip failed. Stderr: {stderr.decode()[:200]}", node="IPTV_CLIPPER")
        except Exception as e:
            colony_log(f"[-] IPTV_CLIPPER ERROR: {e}", node="IPTV_CLIPPER")
