...
async def create_bots(count=5):
    colony_log(f"CREATOR: Generating {count} new high-fidelity bots...", node="CREATOR")
    ...
    while created < count:
        ...
        try:
            ...
            colony_log(f"SUCCESS: Created Bot [ {persona['name']} ] - ID: {bot_id}", node="CREATOR")
            created += 1
    ...
    conn.close()
    colony_log(f"CREATOR: Batch mission complete. {created} bots added to the colony.", node="CREATOR")
