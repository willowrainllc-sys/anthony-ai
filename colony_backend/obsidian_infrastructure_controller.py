...
class ObsidianInfraController:
    ...
    def register_hub_ports(self, server_id: str, count: int = 10):
        ...
        colony_log(f" INFRA_CTRL: Registered {count} hub ports on {server_id}.", node="INFRA_CTRL")
    ...
    def allocate_port_to_buyer(self, buyer_name: str, port_count: int = 1) -> List[str]:
        ...
        colony_log(f" INFRA_CTRL: Allocated {len(allocated_endpoints)} ports to [{buyer_name}]", node="INFRA_CTRL")
    ...
    def generate_provider_manifest(self) -> InfrastructureManifest:
        ...
        colony_log(f" INFRA_CTRL: Provider Manifest [{self.manifest_id}] Locked.", node="INFRA_CTRL")
        return manifest
