...
class ObsidianNodeManager:
    ...
    def generate_one_command_installer(self, exit_ip: str = "47.85.50.46") -> str:
        ...
        colony_log(f"NODE_MGR: Generating one-command installer for [{self.node_id}]...", node="NODE_MGR")
    ...
    async def deploy_local_headless_cluster(self, node_count: int = 3):
        ...
        colony_log(f"NODE_MGR: Deploying {node_count}-node internal virtual cluster...", node="NODE_MGR")
        ...
        for i in range(node_count):
            ...
            colony_log(f" NODE_MGR: Node [{sub_id}] provisioned and joined the grid.", node="NODE_MGR")
