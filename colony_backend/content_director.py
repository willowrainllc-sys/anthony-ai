...
class ContentDirector:
    ...
    async def run_autopilot_cycle(self):
        ...
        colony_log(f"DIRECTOR: Starting Intelligent cycle for [{page_id}]", node="DIRECTOR")
        ...
        # 6. FINAL ASSEMBLY
        production_plan['title'] = optimized.get('title', production_plan.get('title', 'Obsidian Burst'))
        ...
        try:
            with db._get_connection() as conn:
                ...
                rows = conn.execute("""
                    ...
                    WHERE empire_events.node=? AND event_type='BURST_SUCCESS'
    ...
    async def run_autonomous_social_loop(self):
        ...
        """
        colony_log("AGENTIC DAEMON: Content Director loop activated. Feeding social hubs...", node="AGENTIC_DAEMON")

        while True:
            try:
                colony_log(f"AGENTIC DAEMON: Initiating Full Autopilot Production Cycle...", node="AGENTIC_DAEMON")

                # Run the full intelligent cycle
                production_plan = await self.run_autopilot_cycle()

                colony_log(f"AGENTIC DAEMON: Production Plan Generated: {production_plan.get('title')}", node="AGENTIC_DAEMON")
                ...
                colony_log(f"[-] AGENTIC DAEMON ERROR in Content Loop: {e}. Self-healing and continuing...", node="AGENTIC_DAEMON")

            # Wait a few hours before the next autonomous post (simulated here as 4-8 hours)
            sleep_time = random.uniform(3600 * 4, 3600 * 8)
            colony_log(f"AGENTIC DAEMON: Content Director resting. Next production cycle in {sleep_time/3600:.1f} hours.", node="AGENTIC_DAEMON")
