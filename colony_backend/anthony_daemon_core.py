...
class AnthonyChristopherDaemonCore(AnthonyChristopherDaemon):
    ...
    async def run_audit(self):
        colony_log("DAEMON_CORE: Initiating system health audit...", node="DAEMON_CORE")
        # Logic to check port 8000 etc.
        self.send_heartbeat(status="HEALTHY")

    async def run_loop(self):
        colony_log("DAEMON_CORE v4.0 ACTIVE.", node="DAEMON_CORE")
        while True:
            try:
                await self.run_audit()
                await asyncio.sleep(300)
            except Exception as e:
                colony_log(f"ERROR: {e}", node="DAEMON_CORE")
