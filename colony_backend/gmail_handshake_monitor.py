...
class GmailHandshakeMonitor:
    ...
    async def scan_for_b2b_approvals(self, headless: bool = True) -> dict:
        colony_log(f"GMAIL_MONITOR: Scanning inbox for B2B approval signals (Headless={headless})...", node="GMAIL_MONITOR")
        ...
        async with async_playwright() as p:
            ...
            try:
                colony_log("GMAIL_MONITOR: Navigating to search results...", node="GMAIL_MONITOR")
                ...
                await page.screenshot(path="D:\\ObsidianAi_Colony\\Temp\\gmail_b2b_search.png")
                ...
                if found_approval:
                    colony_log(f" GMAIL_MONITOR SUCCESS: Found B2B approval signal from [{found_partner}]!", node="GMAIL_MONITOR")
