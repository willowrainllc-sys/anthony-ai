...
class ProductionWorker(ObsidianDaemon):
    ...
    async def run_worker_loop(self):
        colony_log("WORKER: Autonomous Production Colony active.", node="WORKER")
        ...
        while True:
            try:
                ...
                colony_log(f"[-] WORKER ERROR: {e}", node="WORKER")
    ...
    async def _execute_production_plan(self, job: dict):
        ...
        colony_log(f"WORKER: Launching Production for Job [{job_id}]", node="WORKER")
        ...
        try:
            ...
            for i, scene in enumerate(scenes):
                ...
                if existing_path and os.path.exists(existing_path):
                    colony_log(f"WORKER: Scene {i+1} found in registry. Skipping generation.", node="WORKER")
            ...
            render_path = str(Path(r"D:\ObsidianAi_Colony\Renderings") / f"render_{job_id}.mp4")
            ...
            if success and os.path.exists(render_path):
                ...
                final_path = str(Path(r"D:\ObsidianAi_Colony\Renderings") / f"final_{job_id}.mp4")

                colony_log(f"WORKER: Burning kinetic captions into [{job_id}]...", node="WORKER")
                captioned_path = await caption_engine.generate_captions(render_path, final_path)

                if not captioned_path:
                    colony_log("[-] WORKER: Caption burn failed. Using raw render.", node="WORKER")
                ...
                is_valid, q_msg = await qc_node.verify_burst_readiness(final_path, manifest)
                ...
                if is_valid:
                    ...
                    colony_log(f"WORKER: Mission [{optimized_meta['title']}] SUCCESS.", node="WORKER")
                else:
            ...
            colony_log(f"[-] WORKER CRITICAL: {e}", node="WORKER")
    ...
    async def _generate_scene_and_register(self, scene, bible, key, job_id, index):
        ...
        if not path or not os.path.exists(path):
            colony_log(f"WORKER: Standard provider miss. Engaging VidBee Global Sniper for Scene {index}...", node="WORKER")
        ...
        if not path or not os.path.exists(path):
            colony_log(f"WORKER: Snipe miss. Engaging OpenViking Semantic Fallback for Scene {index}...", node="WORKER")
    ...
    def _reset_stalled_jobs(self):
        with db._get_connection() as conn:
            ...
            conn.commit()
            colony_log("WORKER: Stalled jobs reset to QUEUED.", node="WORKER")
