...
def force_push_all_ready_videos():
    ...
    if not os.path.exists(DB_PATH):
        colony_log(f"[-] SOCIAL_PUSH: Database missing at {DB_PATH}", node="SOCIAL_HUB")
        return

    colony_log("[SUPREME] SOCIAL_PUSH: Unlocking automatic social net dispatch...", node="SOCIAL_HUB")

    conn = sqlite3.connect(DB_PATH)
    cur = conn.cursor()

    # 1. Find all successful burst events with video paths
    ...
    for job_id, title, video_path in jobs:
        try:
            ...
            colony_log(f"[-] SOCIAL_PUSH Error on {job_id}: {e}", node="SOCIAL_HUB")

    conn.close()
    colony_log(f" SOCIAL_PUSH SUCCESS: Dispatched {pushed_count} burst tasks across the hub.", node="SOCIAL_HUB")
    return pushed_count
