...
class QualityGate:
    ...
    def validate_storyboard(storyboard: dict):
        ...
        if errors:
            colony_log(f"QUALITY REJECTION: {len(errors)} issues found.", node="DIRECTOR")
            return False, errors

        colony_log(f"QUALITY GATE: Passed storyboard validation.", node="DIRECTOR")
        return True, []
    ...
    def verify_render(file_path: str, topic: str = None):
        ...
        if not os.path.exists(file_path):
            alt_path = os.path.join(r"D:\ObsidianAi_Colony\Renderings", os.path.basename(file_path))
        ...
        try:
            ...
            if not audio_streams:
                ...
                if "Audio:" not in result_alt.stderr:
                    colony_log(f"QUALITY WARNING: Audio stream missing in {os.path.basename(file_path)}", node="DIRECTOR")
            ...
            colony_log(f"QUALITY ERROR: Technical probe failed: {e}", node="DIRECTOR")
        ...
        colony_log(f"QUALITY GATE: Render verified for {topic}.", node="DIRECTOR")
        return True, []
