...
class AnthonyTitanBrain:
    ...
    3. HIVE COMMAND: Dispatches bursts to the Nest-node mesh.
    ...
    async def _dispatch_agentic_bursts(self, text):
        """Physically modifies the project based on Titan logic."""
        # 1. [SEARCH]
        for query in re.findall(r'\[SEARCH\](.*?)\[/SEARCH\]', text, re.DOTALL):
            colony_log(f"🛰️ TITAN: Scavenging web for [{query.strip()}]", node="INTERNET")
        ...
        for match in re.findall(r'\[WRITE\](.*?)\|(.*?)\[/WRITE\]', text, re.DOTALL):
            ...
            try:
                ...
                colony_log(f"📝 TITAN: Evolved file -> {p.name}", node="SUPREME")
            except: pass

        # 3. [EXECUTE]
        for cmd in re.findall(r'\[EXECUTE\](.*?)\[/EXECUTE\]', text, re.DOTALL):
            colony_log(f"🔥 TITAN: Executing burst -> {cmd.strip()}", node="SUPREME")
