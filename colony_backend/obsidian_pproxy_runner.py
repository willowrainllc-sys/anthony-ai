...
class ObsidianPProxyRunner(ObsidianDaemon):
    ...
    3. AUTO-HEAL: Re-ignites if the Master Port (8000) or Colony Ports (1080+) are blocked.
    """
    def __init__(self):
        super().__init__("PPROXY_RUNNER")
        self.master_port = 8000
        self.colony_start = 1080
        self.colony_end = 1130 # Lean 50-port range for RAM safety
    ...
    def run_matrix(self):
        colony_log("PPROXY_v30: Checking Matrix Integrity...", node="NETWORK")

        while True:
            # Only start if the master port is closed
            if not self._is_port_open(self.master_port):
                colony_log("[-] PPROXY: Port 8000 CLOSED. Re-igniting Obsidian Matrix...", node="NETWORK")
                self._spawn_matrix()
    ...
    def _spawn_matrix(self):
        ...
        for port in range(self.colony_start, self.colony_end + 1):
        ...
        try:
            ...
            colony_log(f" PPROXY SUCCESS: Matrix re-ignited with {len(listeners)} ports. Logging to [{log_file}]", node="NETWORK")
        except Exception as e:
            colony_log(f" PPROXY FATAL: {e}", node="NETWORK")
