...
async def launch_facebook_linker():
    ...
    """
    colony_log("BROWSER: Initializing Facebook Bot Linker...", node="BROWSER")
    ...
    async with async_playwright() as p:
        ...
        colony_log(f"BROWSER: Target ID [{bot_id}]. Opening Facebook...", node="BROWSER")
        ...
        print("4. I will lock the session state for the bot's headless bursts.")
        print("====================================================")
        ...
        colony_log(f" VAULTED: Facebook session secured for {bot_id}.", node="BROWSER")
        ...
        try:
            ...
            conn.execute("""
                ...
                VALUES (?, ?, '6', 'Social-Stealth', 'Facebook Burst', 'ACTIVE', ?)
            """, (bot_id, f"FB_Bot_{bot_id[-4:]}", 0))
            conn.commit()
            conn.close()
            colony_log(f"GRID: {bot_id} is now an active Facebook Disciple.", node="BROWSER")
        except Exception as e:
            colony_log(f"[-] DB_FAIL: {e}", node="BROWSER")
