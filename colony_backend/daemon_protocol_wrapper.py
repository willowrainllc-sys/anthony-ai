...
class DaemonProtocol:
    ...
    async def run_forever(self):
        colony_log(f"DAEMON_PROTOCOL: Establishing permanent life-support for [{self.script_name}]", node="DAEMON")

        while True:
            try:
                if not self._is_running():
                    colony_log(f"[-] DAEMON_PROTOCOL: [{self.script_name}] is offline. Re-igniting...", node="DAEMON")
                    self._start()
                ...
                colony_log(f" DAEMON_PROTOCOL ERROR: {e}", node="DAEMON")
