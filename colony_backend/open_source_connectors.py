...
class ZskyAiAdapter:
    ...
    async def enhance_prompt(self, visual_cue: str) -> str:
        colony_log(f"ZSKY.AI: Enhancing prompt [{visual_cue[:30]}]...", node="ZSKY")
        ...
        try:
            ...
            colony_log(f"ZSKY.AI Note: {e}", node="ZSKY")
...
class SnapGenAiAdapter:
    ...
    def generate_storyboard_frames(self, topic: str, count: int = 3) -> list:
        colony_log(f"SNAPGEN.AI: Synthesizing {count} storyboard frames for [{topic[:20]}]...", node="SNAPGEN")
        frames = []
...
class VibesAiAdapter:
    ...
    def get_vibe_profile(self, niche: str) -> dict:
        colony_log(f"VIBES.AI: Generating aesthetic mood profile for [{niche}]...", node="VIBES")
        vibes = {
