...
class SupabaseMiracleWriter:
    ...
    async def activate_cloud_database(self, headless: bool = False):
        colony_log(f"MIRACLE_WRITER: Initiating Cloud Database Activation (Headless={headless})...", node="SUPABASE_BOT")

        if not SQL_FILE.exists():
            colony_log("[-] MIRACLE_WRITER: empire_master_schema.sql missing!", node="SUPABASE_BOT")
            return False
        ...
        async with async_playwright() as p:
            ...
            try:
                colony_log("MIRACLE_WRITER: Navigating to SQL Portal...", node="SUPABASE_BOT")
                await page.goto(SUPABASE_SQL_URL, timeout=90000, wait_until="networkidle")

                # Give Monaco ample time to mount
                colony_log("MIRACLE_WRITER: Waiting for editor synchronization...", node="SUPABASE_BOT")
                await asyncio.sleep(20)

                # TACTIC 1: Visual Proof
                await page.screenshot(path="D:\\ObsidianAi_Colony\\Temp\\miracle_step1_sync.png")

                # TACTIC 2: Direct JS Model Injection (Fastest & most reliable)
                colony_log("MIRACLE_WRITER: Injecting SQL via internal Monaco API...", node="SUPABASE_BOT")
                injection_js = """
                (content) => {
                    try {
                ...
                colony_log(f"MIRACLE_WRITER: Injection result -> {result}", node="SUPABASE_BOT")

                if "ERROR" in str(result) or "NOT_FOUND" in str(result):
                    colony_log("MIRACLE_WRITER: API injection failed. Falling back to Keyboard brute force...", node="SUPABASE_BOT")
                ...
                await page.screenshot(path="D:\\ObsidianAi_Colony\\Temp\\miracle_step2_populated.png")

                # TACTIC 4: Dual Run Trigger
                colony_log("MIRACLE_WRITER: Triggering SQL BURST...", node="SUPABASE_BOT")
                ...
                colony_log("MIRACLE_WRITER: Monitoring for 'Success' confirmation...", node="SUPABASE_BOT")
                await asyncio.sleep(15)

                # TACTIC 5: Verification Screenshot
                await page.screenshot(path="D:\\ObsidianAi_Colony\\Temp\\miracle_step3_final.png")
                ...
                if "Success" in content or "Query returned no rows" in content or "CREATE TABLE" in content:
                    colony_log(" MIRACLE_WRITER SUCCESS: Cloud Database Activated!", node="SUPABASE_BOT")
                    db.log_event("SUPABASE_BOT", "CLOUD_ACTIVATION_SUCCESS", {"status": "TABLES_PROVISIONED"})
                    await browser.close()
                    return True
                else:
                    colony_log("[-] MIRACLE_WRITER: Verification failed. Check final screenshot.", node="SUPABASE_BOT")
                    await browser.close()
                    return False

            except Exception as e:
                colony_log(f"[-] MIRACLE_WRITER Critical Failure: {e}", node="SUPABASE_BOT")
                await browser.close()
                return False
