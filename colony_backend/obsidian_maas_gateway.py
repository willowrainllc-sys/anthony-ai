...
class ObsidianMaaSGateway:
    ...
    async def issue_maas_subscription_link(self, tier: str = "business") -> dict:
        ...
        colony_log(f"MAAS_GATEWAY: Generating subscription for [{tier_data['name']}] (${tier_data['price']}/mo)...", node="MAAS_GATEWAY")
    ...
    async def process_maas_request(self, payload: MaaSRequest) -> dict:
        ...
        colony_log(f"MAAS_GATEWAY: Processing prompt from client [{payload.api_key[:8]}...] (Complexity: {payload.complexity})", node="MAAS_GATEWAY")
