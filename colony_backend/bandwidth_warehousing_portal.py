...
class BandwidthWarehousingPortal:
    ...
    def allocate_secure_pipeline_access(self, buyer_entity: str, capacity_tb: int = 10) -> WarehousePipelineAccess:
        ...
        colony_log(f"WAREHOUSE: Allocating {capacity_tb} TB secure pipeline from [{warehouse['name']}] for [{buyer_entity}]...", node="WAREHOUSE")
        ...
        colony_log(f" WAREHOUSE SUCCESS: Allocated {capacity_tb} TB pipeline token for {buyer_entity}!", node="WAREHOUSE")
        return access_obj
