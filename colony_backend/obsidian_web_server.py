...
class SovereignWebHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if host in public_domains or is_bridge or target_file.endswith("_landing.html") or target_file == "index.html" or "marketplace" in target_file or target_file == "omni_grid_command.html" or target_file == "enterprise_storefront_hub.html":
            colony_log(f"🌐 PUBLIC INGRESS: Serving [{host}] -> {target_file}", node="SUPREME")
            # Fallback to titan_landing if specific landing doesn't exist yet
            final_path = ROOT / target_file
    ...
    def _handle_apk_download(self):
        ...
        if apk_path.exists() and apk_path.is_file():
            colony_log(f"🌐 WEB: Dispatching APK Burst -> [{apk_path}]", node="SUPREME")
            self._serve_file(apk_path, as_attachment=True, filename="obsidian_global_v26.apk")
        else:
            colony_log(f"[-] WEB ERROR: APK not found at {apk_path}", node="SUPREME")
