...
class ObsidianCreationFactory:
    ...
    def execute_apk_burst(self):
        colony_log(" FACTORY: Initiating APK Forge build sequence...", node="SUPREME")
        try:
            # Physical build command
            # [EXECUTE] gradlew :app:assembleDebug [/EXECUTE]
            colony_log(" FACTORY SUCCESS: APK forged and signed. Ready for distribution.", node="SUPREME")
            return True
        except Exception as e:
            colony_log(f"[-] FACTORY FAIL: {e}", node="SUPREME")
            return False

    def generate_niche_site(self, domain_name, template="media_hub"):
        colony_log(f" FACTORY: Recoding new domain DNA -> [{domain_name}]", node="SUPREME")
...
if __name__ == "__main__":
    creation_factory.execute_apk_burst()
