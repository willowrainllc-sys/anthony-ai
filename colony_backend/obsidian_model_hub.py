...
class ObsidianModelHub:
    ...
    def clone_from_hf_cache(self):
        ...
        colony_log("MODEL_HUB: Scanning legacy HuggingFace cache for cloning...", node="SOVEREIGN")

        if not HUGGINGFACE_CACHE.exists():
            colony_log("[-] MODEL_HUB: HuggingFace cache not found. Vaulting skipped.", node="SOVEREIGN")
            return
        ...
        for item in HUGGINGFACE_CACHE.iterdir():
            if item.is_dir() and item.name.startswith("models--"):
                ...
                if not target_path.exists():
                    colony_log(f"MODEL_HUB: Cloning [{model_name}] to Sovereign Vault...", node="SOVEREIGN")
                    try:
                        shutil.copytree(item, target_path)
                        self._register_model(model_name, str(target_path))
                        cloned_count += 1
                    except Exception as e:
                        colony_log(f"[-] CLONE FAIL [{model_name}]: {e}", node="SOVEREIGN")
                else:
    ...
    def team_get_model_path(self, model_id: str) -> str:
        ...
        with open(self.registry_file, "r") as f:
            ...
            if model:
                colony_log(f"MODEL_HUB: Team access GRANTED for [{model_id}].", node="SOVEREIGN")
                return model["local_path"]

        colony_log(f"[-] MODEL_HUB: Access DENIED. Model [{model_id}] not in Sovereign Vault.", node="SOVEREIGN")
        return None
