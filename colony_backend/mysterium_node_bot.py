...
class MysteriumNodeBot:
    ...
    async def get_node_status_and_myst_balance(self) -> dict:
        colony_log(f"MYSTERIUM_BOT: Connecting to Mysterium Node UI [{self.node_url}]...", node="MYSTERIUM")
        ...
        try:
            async with httpx.AsyncClient(timeout=10.0) as client:
                ...
                if res.status_code == 200:
                    ...
                    colony_log(f" MYSTERIUM_BOT: Node Online! Earned MYST: {myst_earned}", node="MYSTERIUM")
                    return {
                        "status": "NODE_ONLINE",
            ...
            colony_log(f"[-] Mysterium Node UI REST Note: {e}", node="MYSTERIUM")
        ...
        try:
            ...
            colony_log(f"[-] Mysterium Headless Session Note: {e}", node="MYSTERIUM")
    ...
    async def configure_polygon_auto_payout(self, wallet_address: str, auto_payout_myst: float = 5.0) -> dict:
        ...
        colony_log(f"MYSTERIUM_BOT: Setting Polygon payout wallet [{wallet_address[:10]}...] with threshold {auto_payout_myst} MYST...", node="MYSTERIUM")
