...
class VidBeeSnipingEngine:
    ...
    async def snipe_global_asset(self, query: str, site_filter: str = "all") -> dict:
        ...
        """
        colony_log(f"VIDBEE_SNIPER: Scoping 1,000+ sites for visual signal [{query}]...", node="VIDBEE")
        ...
        try:
            colony_log(f"VIDBEE_SNIPER: Handshaking with [{site_filter.upper()}] sources...", node="VIDBEE")
            process = await asyncio.create_subprocess_exec(*cmd)
            await asyncio.wait_for(process.wait(), timeout=180.0)

            if output_path.exists():
                size_mb = round(output_path.stat().st_size / (1024 * 1024), 2)
                colony_log(f" VIDBEE SUCCESS: Sniped {size_mb} MB asset from Global Grid.", node="VIDBEE")
            ...
            colony_log(f"[-] VIDBEE ERROR: {e}", node="VIDBEE")
