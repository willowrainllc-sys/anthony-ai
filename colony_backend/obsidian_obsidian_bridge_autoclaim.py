...
sys_path = str(ROOT / "colony_backend")
...
class ObsidianBridgeAutoClaimEngine:
    ...
    async def audit_withdrawal_readiness(self):
        colony_log("JMPT_BURST: Executing deep dashboard scrape...", node="FINANCE")

        if not SESSION_FILE.exists():
            colony_log(f"[-] JMPT_BURST: Missing session file at {SESSION_FILE}", node="FINANCE")
        ...
        async with async_playwright() as p:
            try:
                ...
                colony_log(f"JMPT_BURST: Verified Credits: {current_credits} / 1,000", node="FINANCE")

                if current_credits >= self.credit_threshold:
                    colony_log("[SUPREME] JMPT_BURST: THRESHOLD REACHED. Forcing physical Bitcoin sweep...", node="FINANCE")
                    ...
                    if await withdraw_btn.is_visible():
                        ...
                        colony_log(f" JMPT_BURST SUCCESS: Sent to {self.btc_target}.", node="FINANCE")
                        await browser.close()
                ...
                colony_log(f"[-] JMPT_BURST Error: {e}", node="FINANCE")
                return {"status": "ERROR"}

    async def execute_payout_burst(self):
...
if __name__ == "__main__":
    async def run():
        res = await jmpt_autoclaim.execute_payout_burst()
