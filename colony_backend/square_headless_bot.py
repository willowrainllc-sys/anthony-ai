...
class SquareHeadlessBot:
    ...
    async def run_square_check(self, headless: bool = True) -> dict:
        colony_log(f"SQUARE_BOT: Initializing Headless Session (Headless={headless})...", node="SQUARE_BOT")

        vaulted_state = cookie_monster.get_vaulted_cookies(self.portal_id)
        if not vaulted_state:
            colony_log("SQUARE_BOT: No Square cookies found. Falling back to Gmail session...", node="SQUARE_BOT")
        ...
        async with async_playwright() as p:
            ...
            if vaulted_state:
                colony_log("SQUARE_BOT: Injecting vaulted session cookies...", node="SQUARE_BOT")
                await context.add_cookies(vaulted_state.get("cookies", []))

            page = await context.new_page()

            try:
                colony_log(f"SQUARE_BOT: Navigating to Square Dashboard...", node="SQUARE_BOT")
                ...
                colony_log(f" SQUARE_BOT: Connected to [{title}] at {url}", node="SQUARE_BOT")

                # Check if we are actually logged in
                if "login" in url.lower() or "signin" in url.lower():
                    colony_log("[-] SQUARE_BOT: Session Expired or Not Logged In. Needs Interactive Login.", node="SQUARE_BOT")
                    await browser.close()
                    return {
                        "status": "NEEDS_LOGIN",
                        "url": url,
                ...
                colony_log(f"[-] SQUARE_BOT Exception: {e}", node="SQUARE_BOT")
                await browser.close()
    ...
    async def run_interactive_login(self):
        ...
        colony_log("SQUARE_BOT: Launching interactive login session. Please log in manually in the browser window.", node="SQUARE_BOT")
            ...
            while browser.is_connected():
                ...
                try:
                    if "dashboard" in page.url.lower() and not ("login" in page.url.lower()):
                        ...
                        state = await context.storage_state()
                        cookie_monster.eat_and_vault_session(self.portal_id, state)
                        colony_log(" SQUARE_BOT: Dashboard reached! Session vaulted successfully.", node="SQUARE_BOT")
                except: break

            colony_log("SQUARE_BOT: Interactive session closed.", node="SQUARE_BOT")
