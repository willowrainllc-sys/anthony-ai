...
class ObsidianActiveDefense:
    ...
    async def log_suspicious_activity(self, ip_address: str, bot_id: str = "Unknown"):
        colony_log(f" DEFENSE: Suspicious activity detected from [{ip_address}]. Sector: {bot_id}", node="SECURITY")

        count = self.attacker_registry.get(ip_address, 0) + 1
        self.attacker_registry[ip_address] = count

        if count >= self.attack_threshold:
            await self._trigger_counter_burst(ip_address)

    async def _trigger_counter_burst(self, target_ip: str):
        colony_log(f" COUNTER_BURST: ATTACK CONFIRMED from [{target_ip}]. Deploying Toxic Payload...", node="SECURITY")
        ...
        db.log_event("SECURITY", "COUNTER_BURST_DEPLOYED", {"target": target_ip, "payload": "TOXIC_DATA_STREAM"})

        # 2. Local Lockdown
        from obsidian_kill_switch import kill_switch
        # We don't kill the whole grid, just the compromised sector
        colony_log(f" LOCKDOWN: Sector targeting [{target_ip}] is now GHOSTED.", node="SECURITY")

        # 3. Report to Global Grid
        colony_log(f" DEFENSE SUCCESS: Attacker [{target_ip}] has been neutralized and blacklisted.", node="SECURITY")
    ...
    def execute_data_incineration(self):
        ...
        colony_log(" SELF_DESTRUCT: All sensitive digital traces incinerated.", node="SECURITY")
