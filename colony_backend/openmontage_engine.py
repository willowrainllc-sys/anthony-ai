...
class OpenMontageEngine:
    ...
    def create_montage_sequence(
        ...
        colony_log(f"OPENMONTAGE: Assembling open-source Python video montage across {len(clip_paths)} source assets...", node="OPENMONTAGE")
        ...
        try:
            ...
            colony_log(f" OPENMONTAGE SUCCESS: Compiled {len(processed_montage_clips)} smooth montage sequence clips!", node="OPENMONTAGE")
            return processed_montage_clips

        except Exception as e:
            import traceback
            colony_log(f"[-] OpenMontage Error: {e}\n{traceback.format_exc()}", node="OPENMONTAGE")
            return []
