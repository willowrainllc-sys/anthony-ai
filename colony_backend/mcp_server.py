...
@mcp.tool()
async def trigger_longform_burst(publish_live: bool = False) -> dict:
    from longform_production_orchestrator import longform_orchestrator
    return await longform_orchestrator.execute_longform_burst(publish_live=publish_live)

@mcp.tool()
async def trigger_master_burst(category: str = "exoplanetary_anomalies", target_min: int = 1) -> dict:
...
@mcp.tool()
async def trigger_mastermind_trade_burst() -> dict:
    """
    Executes a high-conviction trade burst using the Trading Mastermind Team.
    ...
    try:
        ...
        return await trading_mastermind.execute_obsidian_trade_burst()
    except Exception as e:
...
async def trigger_mass_linking_burst(limit: int = 50) -> dict:
    ...
    try:
        ...
        count = await linker.execute_mass_linking_burst(limit)
        return {"status": "SUCCESS", "accounts_linked": count}
    except Exception as e:
        return {"status": "error", "message": str(e)}

@mcp.tool()
async def trigger_wholesale_market_burst() -> dict:
    """
    ...
    try:
        ...
        count = await market_scout.scout_and_burst_wholesale()
...
async def trigger_global_termination() -> dict:
    """
    Kills all active colony processes and collapses the grid.
...
@mcp.tool()
async def trigger_high_greed_burst(channel: str = "PINTEREST") -> dict:
    from obsidian_revenue_multiplier import revenue_multiplier
    return await revenue_multiplier.generate_high_greed_marketing_burst(channel)
