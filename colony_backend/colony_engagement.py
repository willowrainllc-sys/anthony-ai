...
def fetch_active_disciples():
    ...
    try:
        ...
        colony_log(f"[-] DB Disciple Fetch Error: {e}", node="COLONY")
        return [
...
async def boost_live_post(post_url: str, post_title: str, platform: str):
    """
    DISCIPLE COLONY BOOST PROTOCOL:
    ...
    colony_log(f"COLONY BOOST: Mobilizing {len(disciples)} Disciples for live drop: [{post_title[:30]}] on {platform}", node="COLONY")
    ...
    for disc in active_wave:
        try:
            ...
            colony_log(f"DISCIPLE [{disc['name']}]: Pulse-Locked -> Comment: \"{comment}\"", node="COLONY")

            # Log engagement pulse to Vault
            db.log_event("COLONY", "DISCIPLE_ENGAGEMENT_SUCCESS", {
                "disciple_id": disc['id'],
            ...
            colony_log(f"[-] Disciple Engagement Error for {disc['name']}: {e}", node="COLONY")

async def monitor_empire_signals():
    """Listens for live BURST_SUCCESS events and triggers the Disciple Colony Boost."""
    colony_log(" Disciple Colony Engine: Active Signal Monitor online.", node="COLONY")
        ...
        try:
            with db._get_connection() as conn:
                rows = conn.execute("""
                    ...
                    WHERE event_type='BURST_SUCCESS' AND id > ?
            ...
            colony_log(f"[-] Signal Monitor Error: {e}", node="COLONY")
