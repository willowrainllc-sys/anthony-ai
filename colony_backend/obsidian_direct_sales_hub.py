...
class ObsidianDirectSalesHub:
    ...
    async def create_direct_b2b_contract(self, company_name: str, service: str = "PRIVATE_PROXY_PORT") -> ServiceContract:
        colony_log(f"DIRECT_SALES: Drafting direct contract for [{company_name}]...", node="SALES_HUB")
        ...
        colony_log(f" SALES_HUB SUCCESS: Direct contract [{contract_id}] live for [{company_name}]! Fee: ${fee:,.2f} USD.", node="SALES_HUB")
        return contract
