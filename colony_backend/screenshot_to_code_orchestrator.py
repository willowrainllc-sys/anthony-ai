...
class ScreenshotToCodeOrchestrator:
    ...
    async def capture_target_ui(self, url: str) -> str:
        ...
        colony_log(f"UI_CODE: Capturing target UI from [{url}]...", node="CODE_GEN")
        ...
        async with async_playwright() as p:
            ...
            try:
                ...
                colony_log(f" UI_CODE: Screenshot captured at {shot_path.name}", node="CODE_GEN")
                return str(shot_path)
            except Exception as e:
                colony_log(f"[-] UI_CODE Capture Error: {e}", node="CODE_GEN")
                return ""
            finally:
                await browser.close()
    ...
    async def generate_code_from_screenshot(self, screenshot_path: str, tech_stack: str = "HTML + Tailwind") -> dict:
        ...
        colony_log(f"UI_CODE: Analyzing visual structure for [{tech_stack}] generation...", node="CODE_GEN")
        ...
        colony_log(f" UI_CODE SUCCESS: Code generated and saved to {code_path.name}", node="CODE_GEN")
