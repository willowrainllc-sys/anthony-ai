...
class MasterStudioFactory:
    ...
    async def produce_and_dispatch_episode(
        ...
        colony_log(f"STUDIO: Initiating A-Roll + B-Roll Production [{category.upper()} | {target_min} MIN]...", node="STUDIO")
        ...
        if self._is_duplicate_upload(title):
            colony_log(f"STUDIO: Title [{title}] already published. Pulling fresh topic...", node="STUDIO")
        ...
        try:
            ...
            colony_log(f"STUDIO Supabase Sync Note: {e}", node="STUDIO")
        ...
        db.log_event("STUDIO", "BURST_SUCCESS", {
        ...
        colony_log(f"[SUPREME] A-ROLL + B-ROLL PRODUCTION COMPLETE! YouTube Live: {yt_url}", node="STUDIO")
