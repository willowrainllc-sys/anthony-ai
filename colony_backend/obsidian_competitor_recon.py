...
class CompetitorReconBurst:
    """
    COMPETITOR RECON BURST:
    ...
    async def execute_recon_burst(self):
        colony_log("RECON: Initiating Global Competitor Intelligence Burst...", node="COMMAND")
        ...
        async with async_playwright() as p:
            ...
            for industry, url in self.targets.items():
                colony_log(f"RECON: Capturing [{industry}] leader DNA -> {url}", node="COMMAND")
                ...
                try:
                    ...
                    colony_log(f"[-] RECON FAIL [{industry}]: {e}", node="COMMAND")

                await page.close()

            await browser.close()

        colony_log("[SUPREME] RECON: All competitor DNA secured in vault.", node="COMMAND")

if __name__ == "__main__":
    recon = CompetitorReconBurst()
    asyncio.run(recon.execute_recon_burst())
