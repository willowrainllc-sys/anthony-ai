...
class ObsidianNodeIgniter:
    ...
    async def ignite_node(self, port: int):
        ...
        colony_log(f"IGNITER: Powering up Node [{node_id}] via {proxy_url}...", node="SUPREME")
        ...
        async with async_playwright() as p:
            try:
                ...
                if "dashboard" in page.url:
                    colony_log(f" IGNITER: Node [{node_id}] is officially CLOCKING IN.", node="SUPREME")
    ...
    async def execute_mass_ignition(self):
        ...
        colony_log(f"IGNITER: Mobilizing {len(ports)} real nodes to hit the 1,000 credit mark...", node="SUPREME")
        ...
        for i in range(0, len(ports), 5):
            ...
            colony_log(f" IGNITER: {i+5}/{len(ports)} nodes activated.", node="SUPREME")
