...
class ObsidianCapitalFlipEngine:
    ...
    async def monitor_and_flip_gains(self):
        colony_log("FLIP_ENGINE: Auditing realized gains for zero-loss compounding...", node="CAPITAL_FLIP")
        ...
        if realized_profit >= MIN_FLIP_THRESHOLD:
            ...
            colony_log(f"[SUPREME] FLIP_ENGINE: Realized Profit Detected [${realized_profit:,.2f}]. Initiating ${flip_amount:,.2f} Flip Burst...", node="CAPITAL_FLIP")
            ...
            await revenue_multiplier.generate_high_greed_marketing_burst(channel="YOUTUBE")
            ...
            colony_log(f" FLIP_ENGINE SUCCESS: Real capital building. ${flip_amount:,.2f} has been put back to work.", node="CAPITAL_FLIP")
            return {"status": "SUCCESS", "flipped": flip_amount}

        colony_log("FLIP_ENGINE: Gains below threshold. Holding for accumulation.", node="CAPITAL_FLIP")
