...
class ObsidianWebSearch:
    ...
    async def search_live_web(self, query: str, max_results: int = 5) -> List[Dict[str, str]]:
        colony_log(f"WEB_SEARCH: Scouting the internet for [{query}]...", node="INTERNET")
        ...
        try:
            ...
            colony_log(f" WEB_SEARCH SUCCESS: Captured {len(results)} live internet signals.", node="INTERNET")

            db.log_event("INTERNET", "WEB_SEARCH_COMPLETE", {"query": query, "count": len(results)})
            return results

        except Exception as e:
            colony_log(f"[-] WEB_SEARCH ERROR: {e}", node="INTERNET")
            return []
