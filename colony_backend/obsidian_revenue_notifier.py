...
class RevenueNotifier:
    ...
    async def pulse_revenue_event(self, amount: float, source: str):
        colony_log(f" NOTIFIER: REAL MONEY DETECTED! ${amount:,.2f} from [{source}].", node="NOTIFIER")
        ...
        colony_log(f" NOTIFIER SUCCESS: Signal dispatched to Director's HUD.", node="NOTIFIER")
