...
class FastMcpSubscriptionManager:
    ...
    async def generate_api_subscription_checkout(self, plan_type: str = "starter") -> dict:
        ...
        colony_log(f"FASTMCP_SUB: Generating Square subscription checkout for [{plan_name}] (${price}/mo)...", node="FASTMCP_SUB")
    ...
    def issue_developer_api_key(self, customer_email: str, plan_type: str = "starter") -> DeveloperApiKey:
        ...
        colony_log(f" FASTMCP_SUB: Issued new API key [{api_key[:16]}...] for {customer_email}", node="FASTMCP_SUB")
        return key_obj
