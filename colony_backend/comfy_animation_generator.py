...
OUTPUT_DIR = Path(r"D:\ObsidianAi_Colony\Renderings")
...
class ComfyAnimationGenerator:
    ...
    async def render_animation_clip(self, prompt: str, duration_sec: int = 5, aspect_ratio: str = "9:16") -> str:
        ...
        """
        colony_log(f"COMFYUI: Checking local animation server status...", node="COMFY")
        if not await self.is_comfy_online():
            colony_log("[-] COMFYUI: Local server not running on port 8188. Bypassing animation node.", node="COMFY")
            return None

        width, height = (512, 896) if aspect_ratio == "9:16" else (896, 512)
        frames = int(duration_sec * 16) # 16 fps animation

        colony_log(f"COMFYUI: Submitting animation job for [{prompt[:30]}] ({frames} frames)...", node="COMFY")
        ...
        try:
            async with httpx.AsyncClient(timeout=180.0) as client:
                ...
                if res.status_code == 200:
                    ...
                    colony_log(f" COMFYUI: Prompt queued with ID [{prompt_id}]. Waiting for render...", node="COMFY")
                    return prompt_id
        except Exception as e:
            colony_log(f"[-] COMFYUI Render Error: {e}", node="COMFY")
