...
class QualityControlNode:
    ...
    """
    async def verify_burst_readiness(self, video_path: str, manifest: dict):
        colony_log(f"QC: Inspecting production [{os.path.basename(video_path)}]...", node="QC")
        ...
        if not tech_pass:
            colony_log(f"QC REJECTION [TECH]: {tech_msg}", node="QC")
            return False, tech_msg

        # 2. CREATIVE CHECK
        creative_pass, creative_msg = await self._run_creative_check(video_path, manifest)
        if not creative_pass:
            colony_log(f"QC REJECTION [CREATIVE]: {creative_msg}", node="QC")
            return False, creative_msg

        colony_log("QC: Burst readiness VERIFIED.", node="QC")
        return True, "Passed"
    ...
    async def _run_technical_check(self, video_path: str):
        try:
            ...
            if "Audio:" not in output:
                colony_log("QC: Missing audio stream. Injecting audio track...", node="QC")
                await self._add_silent_audio(video_path)

            return True, "Technical OK"
        except Exception as e:
            colony_log(f"[-] QC PROBE FAIL: {e}", node="QC")
    ...
    async def _add_silent_audio(self, video_path: str):
        try:
            ...
            colony_log(f"[-] Audio Injection Error: {e}", node="QC")
