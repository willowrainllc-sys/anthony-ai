...
class ObsidianKillSwitch:
    ...
    1. PROCESS PURGE: Kills all active Python and Docker processes related to the colony.
    ...
    async def execute_global_termination(self):
        colony_log("[ALERT] KILL_SWITCH: INITIATING GLOBAL GRID TERMINATION...", node="SECURITY")

        # 1. Kill all Python processes (broad but effective for instant shutdown)
        try:
            subprocess.run("taskkill /F /IM python.exe", shell=True, capture_output=True)
            colony_log(" KILL_SWITCH: All Python workers terminated.", node="SECURITY")
        except: pass

        # 2. Kill Docker containers (if any)
        try:
            subprocess.run("docker kill $(docker ps -q)", shell=True, capture_output=True)
            colony_log(" KILL_SWITCH: All virtual nodes collapsed.", node="SECURITY")
        except: pass
