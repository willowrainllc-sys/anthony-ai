...
class PawnsBrowserBot:
    ...
    async def run_pawns_check(self, headless: bool = True) -> dict:
        colony_log("PAWNS_BOT: Initializing Pawns.app Headless Session...", node="PAWNS_BOT")
        try:
            ...
            async with async_playwright() as p:
                ...
                colony_log("PAWNS_BOT: Navigating to Pawns.app dashboard...", node="PAWNS_BOT")
                await page.goto(self.url, timeout=30000)
                await asyncio.sleep(3)

                title = await page.title()
                colony_log(f" PAWNS_BOT: Connected to portal [{title}]", node="PAWNS_BOT")
            ...
            colony_log(f"[-] PAWNS_BOT Note: {e}", node="PAWNS_BOT")
            return {
