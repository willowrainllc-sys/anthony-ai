...
class ObsidianMilestoneBilling:
    ...
    async def generate_milestone_invoice(self, client: str, milestone_key: str) -> MilestoneInvoice:
        ...
        colony_log(f"BILLING: Generating invoice for [{client}] - [{project}]...", node="BILLING")
        ...
        colony_log(f" BILLING SUCCESS: Invoice [{invoice_id}] sent to [{client}]. Value: ${amount:,.2f} USD.", node="BILLING")
        return invoice
