        ...
        try:
            ...
            with db._get_connection() as conn:
                recent = conn.execute("""
                    ...
                    WHERE node='YOUTUBE' AND event_type='BURST_SUCCESS'
            ...
            colony_log(f"ANALYTICS ERROR [YT]: {e}", node="ANALYTICS")

    async def _update_creative_memory(self):
        """Updates content_profiles with successful hooks/topics."""
        colony_log("ANALYTICS: Updating Grid Memory...", node="ANALYTICS")
        try:
            with db._get_connection() as conn:
                ...
                for plat, job_id, views in best_performing:
                    ...
                    colony_log(f"MEMORY: Job [{job_id}] on {plat} is a Win ({views} views).", node="ANALYTICS")
        except: pass
