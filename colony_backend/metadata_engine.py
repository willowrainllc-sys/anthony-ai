...
class MetadataEngine:
    ...
    async def optimize_for_grid(self, script_body: str, niche: str, original_title: str):
        colony_log(f"METADATA: Architecting multi-platform distribution for [{original_title[:20]}]", node="METADATA")
        ...
        if not res or len(res) < 5:
            colony_log("METADATA: Brain returned empty. Using safe defaults.", node="METADATA")
            return {
        ...
        try:
            ...
            colony_log(f"METADATA: Parse error [{e}]. Using safe defaults.", node="METADATA")
