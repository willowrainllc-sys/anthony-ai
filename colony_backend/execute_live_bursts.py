...
async def ensure_public_url(media_ptr, channel):
    ...
    if os.path.exists(media_ptr) and supabase:
        try:
            ...
            colony_log(f"[-] SYNC ERROR: {e}", node="CORE")
...
def is_facebook_posting_allowed() -> bool:
    ...
    if current_hour >= 0 and current_hour < 5:
        colony_log("FB PACING: Cooldown window active (12 AM - 5 AM). Skipping Facebook burst.", node="CORE")
        return False
    ...
    try:
        ...
        with db._get_connection() as conn:
            ...
            daily_count = conn.execute(
                "SELECT COUNT(*) FROM empire_events WHERE node='FACEBOOK' AND event_type='BURST_SUCCESS' AND timestamp > ?",
                (day_cutoff,)
            ).fetchone()[0]

            if daily_count >= 10:
                colony_log(f"FB PACING: Daily limit reached ({daily_count}/10 posts). Skipping Facebook burst.", node="CORE")
                return False
            ...
            if window_count >= 2:
                colony_log(f"FB PACING: 3-Hour limit reached ({window_count}/2 posts). Skipping Facebook burst.", node="CORE")
                return False

    except Exception as e:
        colony_log(f"FB PACING ERR: {e}", node="CORE")
...
async def publish_to_facebook(task, token, page_id, video_url, title):
    ...
    try:
        async with httpx.AsyncClient(timeout=300.0) as client:
            colony_log(f"BURST: Initializing FB Reel on Page {page_id}...", node="CORE")
            ...
            if start_res.status_code == 200:
                ...
                colony_log(f"FB: Uploading binary payload ({len(v_data)} bytes)...", node="CORE")
                headers = {
                ...
                if up_res.status_code == 200:
                    colony_log("FB: Data received by Meta. Finishing publish...", node="CORE")
                    ...
                    if fin_res.status_code == 200:
                        ...
                        colony_log(f"[SUPREME] SUCCESS: Facebook Reel Live! URL: {post_url}", node="CORE")
                        return post_url
                    else:
                        colony_log(f"[-] FB Finish Fail: {fin_res.text}", node="CORE")
                else:
                    colony_log(f"[-] FB Rupload Fail: {up_res.text}", node="CORE")
            else:
                colony_log(f"[-] FB Start Fail: {start_res.text}", node="CORE")
    except Exception as e:
        colony_log(f"[-] Facebook Burst Exception: {e}", node="CORE")
    return False

async def publish_to_insta(task, token, ig_business_id, video_url, title, description):
    """v3.7: Instagram Reel Burst via Graph API."""
    try:
        async with httpx.AsyncClient(timeout=180.0) as client:
            colony_log(f"BURST: Launching IG Reel on Business ID {ig_business_id}...", node="CORE")
            ...
            if res.status_code == 200:
                cid = res.json().get("id")
                colony_log(f"IG: Container {cid} created. Waiting for ingestion...", node="CORE")
                ...
                if pub_resp.status_code == 200:
                    colony_log("[SUPREME] SUCCESS: IG Reel Published.", node="CORE")
                    return True
            else:
                colony_log(f"[-] IG Init Fail: {res.text}", node="CORE")
    except Exception as e:
        colony_log(f"[-] IG Exception: {e}", node="CORE")
    return False
...
async def publish_to_threads(video_url, title, description=""):
    ...
    if not threads_id or not threads_token:
        colony_log("[-] Threads: Missing THREADS_USER_ID or THREADS_ACCESS_TOKEN", node="CORE")
        return False
    try:
        async with httpx.AsyncClient(timeout=180.0) as client:
            colony_log(f"BURST: Launching Threads Video Drop on ID {threads_id}...", node="CORE")
            ...
            if t_res.status_code == 200:
                ...
                colony_log(f"Threads: Container {tid} created. Waiting 45s...", node="CORE")
                ...
                if pub_res.status_code == 200:
                    colony_log("[SUPREME] SUCCESS: Threads Burst Live.", node="CORE")
                    return True
                else:
                    colony_log(f"[-] Threads Publish Fail: {pub_res.text}", node="CORE")
            else:
                colony_log(f"[-] Threads Init Fail: {t_res.text}", node="CORE")
    except Exception as e:
        colony_log(f"[-] Threads Exception: {e}", node="CORE")
    return False

async def process_channel_loop(channel):
    colony_log(f"GRID: Node {channel} sequence established.", node="CORE")
    while True:
        try:
            ...
            # --- DUPLICATION SHIELD ---
            if db.is_duplicate_burst(channel, title):
                colony_log(f"SHIELD: Duplicate burst detected for [{title[:30]}] on {channel}. Aborting.", node="CORE")
                db.complete(task['id']) # Mark as complete to move on
                continue

            if not video_url or len(str(video_url)) < 10:
                colony_log(f"BURST: No video URL for [{title[:20]}]. Generating cinematic package...", node="CORE")
                filename = await create_reel_package(title, json.dumps(payload))
                if filename:
                    video_url = str(Path(r"D:\ObsidianAi_Colony\Renderings") / filename)
                else:
                    db.fail(task['id'], "Rendering engine failed to produce asset.")
                    continue

            # Pre-Burst QC
            is_valid, q_msg = await qc_node.verify_burst_readiness(video_url, payload)
            if not is_valid:
            ...
            if channel == "ANTHONY_AI_APP":
                ...
                colony_log("TIKTOK: Handing over to specialized TikTok node (Playwright required).", node="CORE")
            ...
            if success:
                ...
                db.log_event(channel, "BURST_SUCCESS", {
                    "title": title,
                    "video_url": public_url,
                    "post_url": final_post_url or public_url
                })
                colony_log(f"[SUPREME] BURST COMPLETE: [{title[:30]}] on {channel}", node="CORE")
                await asyncio.sleep(120) # 2 min cooling instead of 10
            else:
                db.fail(task['id'], "API rejection or sync failure")

        except Exception as e:
            colony_log(f"[-] {channel} Loop Error: {e}", node="CORE")
            await asyncio.sleep(60)

async def main():
    colony_log("PRODUCTION WORKER: v3.7 High-Performance Active.", node="CORE")

    # Reset stalled tasks
    try:
        from colony_persistence import db
        with db._get_connection() as conn:
            conn.execute("UPDATE colony_tasks SET status='NEGOTIATING' WHERE status='PROCESSING'")
            conn.commit()
            colony_log("GRID: Stalled tasks reset to NEGOTIATING.", node="CORE")
    except Exception as e:
        colony_log(f"[-] Reset Fail: {e}", node="CORE")
