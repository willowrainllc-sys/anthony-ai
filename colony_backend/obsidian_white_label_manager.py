...
class ObsidianWhiteLabelManager:
    ...
    def onboard_new_reseller(self, name: str, email: str) -> dict:
        ...
        colony_log(f" WHITE_LABEL: Onboarded partner [{name}]. Issued Reseller ID: {reseller_id}", node="RESELLER_HUB")
    ...
    async def calculate_network_splits(self, total_burst_yield: float) -> dict:
        ...
        total_reseller_share = total_burst_yield * 0.50
        willow_rain_net = total_burst_yield * 0.50

        colony_log(f"RESELLER_HUB: Calculating revenue splits for burst yield [${total_burst_yield:,.2f}]", node="RESELLER_HUB")

        return {
            "total_yield_usd": total_burst_yield,
    ...
    def generate_grand_reseller_manifest(self) -> WhiteLabelManifest:
        ...
        colony_log(f" WHITE_LABEL: Reseller Manifest Locked. Network Scale: {len(self.resellers)} Partners.", node="RESELLER_HUB")
        return manifest
