...
class MultiInterfaceNetworkRouter:
    ...
    def refresh_interfaces(self) -> list:
        ...
        try:
            ...
            colony_log(f"[-] Interface Scan Note: {e}", node="ROUTER")

        colony_log(f" ROUTER: Detected {len(self.active_interfaces)} active local network interface(s).", node="ROUTER")
    ...
    async def probe_interface_outbound_speed(self, local_ip: str) -> dict:
        ...
        try:
            ...
            colony_log(f"[-] Interface Probe Note for {local_ip}: {e}", node="ROUTER")
