...
class HumanStealthHelper:
    ...
    async def apply_human_jitter(min_sec: float = 2.0, max_sec: float = 6.0):
        ...
        colony_log(f"STEALTH: Applying human jitter delay ({delay}s)...", node="STEALTH")
    ...
    async def robust_click_element(page, selector_or_testid: str):
        ...
        try:
            ...
            if box:
                ...
                await page.mouse.up()
                colony_log(f" STEALTH: Executed robust Bezier click on [{selector_or_testid}] at ({target_x}, {target_y})", node="STEALTH")
                return True
        except Exception as e:
            colony_log(f"[-] Robust Click Fallback for {selector_or_testid}: {e}", node="STEALTH")
            try:
    ...
    async def inject_stealth_scripts(page):
        ...
        try:
            ...
            colony_log(" STEALTH: Anti-bot masking scripts injected into page context.", node="STEALTH")
        except Exception as e:
            colony_log(f"[-] Stealth Injection Note: {e}", node="STEALTH")
    ...
    async def handle_interruptions(page) -> str:
        ...
        try:
            ...
            if any(phrase in content_lower for phrase in ["verification code", "two-factor authentication", "enter the code", "confirm it's you", "prove you're human", "security check"]):
                colony_log("[-] AGENTIC WALL DETECTED: 2FA or Security Challenge. Manual intervention or cookie refresh required.", node="AGENTIC_DAEMON")
                return "NEEDS_2FA"
            ...
            for text in dismiss_buttons:
                try:
                    ...
                    if await btn.is_visible(timeout=1000):
                        colony_log(f"AGENTIC DAEMON: Self-healing popup... clicking '{text}' to dismiss overlay.", node="AGENTIC_DAEMON")
                        await btn.click()
            ...
            colony_log(f"[-] AGENTIC INTERRUPTION ERROR: {e}", node="AGENTIC_DAEMON")
            return "ERROR"
    ...
    async def take_learning_snapshot(page, bot_name: str, state_name: str):
        ...
        try:
            ...
            colony_log(f"AGENTIC DAEMON: Hardcoded learning snapshot saved for [{bot_name}] -> [{state_name}]", node="AGENTIC_DAEMON")
        except Exception as e:
            colony_log(f"[-] AGENTIC SNAPSHOT FAILED: {e}", node="AGENTIC_DAEMON")
