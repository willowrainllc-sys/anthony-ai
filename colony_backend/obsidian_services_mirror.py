...
class ServicesMirrorBurst:
    """
    SERVICES MIRROR BURST:
    ...
    async def execute_mirror(self):
        colony_log("MIRROR: Initiating Services (Angi/Thumbtack) DNA Burst...", node="COMMAND")
        ...
        async with async_playwright() as p:
            ...
            for url in self.targets:
                ...
                try:
                    colony_log(f"[*] MIRROR: Capturing {name.upper()} DNA...", node="COMMAND")
                    await page.goto(url, timeout=60000, wait_until="networkidle")
                    await page.screenshot(path=RECON_DIR / f"{name}_landing.png", full_page=True)
                except Exception as e:
                    colony_log(f"[-] MIRROR FAIL [{name}]: {e}", node="COMMAND")
                await page.close()
    ...
    async def _build_town360_services(self):
        ...
        colony_log("MIRROR: Architecting 'TOWN 360 Services' Marketplace...", node="COMMAND")
...
<head>
    ...
    <style>
        ...
        .btn-burst { background: #3b82f6; color: #fff; padding: 12px 24px; border-radius: 6px; font-weight: 800; text-transform: uppercase; }
    </style>
</head>
<body class="p-12">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            ...
            <div class="card p-8">
                ...
                <div class="flex justify-between items-center">
                    ...
                    <button class="btn-burst" onclick="location.href='obsidian_unified_checkout.html?type=service_install&price=150.00'">HIRE_PRO</button>
                </div>
            </div>
            <!-- Service 2 -->
            <div class="card p-8">
                ...
                <div class="flex justify-between items-center">
                    ...
                    <button class="btn-burst" onclick="location.href='obsidian_unified_checkout.html?type=service_smart&price=299.00'">HIRE_PRO</button>
                </div>
            </div>
            <!-- Service 3 -->
            <div class="card p-8">
                ...
                <p class="text-xs text-gray-500 mb-6">Physical and digital perimeter burst analysis.</p>
                <div class="flex justify-between items-center">
                    <span class="text-emerald-500 font-bold">$499+</span>
                    <button class="btn-burst" onclick="location.href='obsidian_unified_checkout.html?type=service_audit&price=499.00'">HIRE_PRO</button>
                </div>
            </div>
        </div>
    </div>
