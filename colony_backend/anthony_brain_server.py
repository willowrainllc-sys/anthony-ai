...
class AnthonyBrainEngine:
    ...
    2. TITAN MASK: Cloaks every inference burst behind the Missouri Root.
    ...
    """
    def __init__(self):
        colony_log(f"BRAIN: Initializing Native Sovereign Base [{MODEL_ID}]...", node="SUPREME")
    ...
    async def _intercept_agentic_tags(self, text):
        ...
        for cmd in re.findall(r'\[EXECUTE\](.*?)\[/EXECUTE\]', text, re.DOTALL):
            colony_log(f"🧠 BRAIN_EXEC: {cmd.strip()}", node="SECURITY")
        ...
        for path, content in re.findall(r'\[WRITE\](.*?)\|(.*?)\[/WRITE\]', text, re.DOTALL):
            colony_log(f"🧠 BRAIN_WRITE: {path.strip()}", node="SECURITY")
