...
class GridSentinel:
    ...
    async def run_defense_loop(self):
        colony_log("[SUPREME] GRID SENTINEL: Disk & RAM Defense ACTIVE.", node="SENTINEL")
        ...
        while True:
            try:
                ...
                if free_gb < 5.0:
                    colony_log(f"SENTINEL: Low Disk Space ({round(free_gb, 1)}GB). Purging debris...", node="SENTINEL")
                    self.purge_debris()

                # 2. RAM DEFENSE
                ram = psutil.virtual_memory()
                if ram.percent > 90:
                    colony_log(f"SENTINEL: CRITICAL RAM PRESSURE ({ram.percent}%). Recommending grid slowdown.", node="SENTINEL")

                await asyncio.sleep(300) # Check every 5 mins
            except Exception as e:
                colony_log(f"[-] SENTINEL ERROR: {e}", node="SENTINEL")
