...
class ObsidianAPIIgniter:
    ...
    async def execute_blitz(self):
        ...
        for i in range(0, len(self.ports), batch_size):
            ...
            colony_log(f" IGNITER: {total_active} nodes ARMED.", node="SUPREME")
            await asyncio.sleep(0.5)

        colony_log(f"[SUPREME] SUPREME SUCCESS: {total_active} REAL nodes feeding your account.", node="SUPREME")
        return total_active
