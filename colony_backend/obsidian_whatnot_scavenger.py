...
class WhatnotScavenger:
    """
    WHATNOT SCAVENGER:
    Automates the 'Watching & Winning' burst for free physical rewards.
    1. ACCOUNT INGRESS: Creates and manages Whatnot accounts via the 5,000 IP Mesh.
    2. GIVEAWAY BURST: Physically enters live giveaways during trending streams.
    ...
    async def run_scavenge_loop(self):
        ...
        async with async_playwright() as p:
            try:
                ...
                colony_log(f"SCAVENGER [{self.node_id}]: Becoming a user on Whatnot...", node="RECON")
                await page.goto("https://www.whatnot.com/login", wait_until="networkidle")

                # 2. Scavenge Trending Streams for Giveaways
                colony_log(f"SCAVENGER [{self.node_id}]: Scanning 2026 trending streams for giveaways...", node="RECON")
                ...
                colony_log(f"[-] SCAVENGER [{self.node_id}] BURST FAIL: {e}", node="RECON")
                await browser.close()
