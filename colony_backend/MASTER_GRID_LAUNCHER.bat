@echo off
TITLE 🔱 EMPIRE COLONY: MASTER GRID LAUNCHER
cd /d "%~dp0"
...
start "📲 FRONTEND DISPATCHER: PHONE POSTING" cmd /k "..\venv\Scripts\python.exe execute_live_bursts.py"
...
:: 6. Ghost Status Burstr (Text Posts)
echo [*] Ghost Status Burstr will run in a separate loop for text updates.
start "🔱 GHOST FRONTEND: STATUS BURSTS" cmd /k "..\venv\Scripts\python.exe ghost_frontend_status_post.py"
