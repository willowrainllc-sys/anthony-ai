...
class ObsidianRobinhoodBridge:
    ...
    async def launch_headed_trade_session(self):
        swarm_log(f"💹 FINANCE: Initiating Headed Robinhood strike on [{self.target_node}]...", node="FINANCE")
        ...
        try:
            cmd = f"python {self.root_dir}/swarm_backend/obsidian_voyager_shell.py --url={self.robinhood_url} --node={self.target_node}"
            subprocess.Popen(f"start /b {cmd}", shell=True)

            swarm_log(f"✓ FINANCE SUCCESS: Robinhood Ingress established. Awaiting Director handshake.", node="FINANCE")
            ...
            swarm_log(f"[-] FINANCE ERROR: {e}", node="FINANCE")
