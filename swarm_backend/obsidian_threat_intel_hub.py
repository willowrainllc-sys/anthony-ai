...
class ObsidianThreatIntelHub:
    ...
    async def harvest_critical_threat_signals(self) -> List[IntelSignal]:
        swarm_log("INTEL_HUB: Scanning the grid for cyber-threat signals...", node="THREAT_HUB")
        ...
        swarm_log(f" THREAT_HUB SUCCESS: Harvested {len(signals)} verified signals. Total Value: ${sum(s.wholesale_value_usd for s in signals):,.2f}", node="THREAT_HUB")
        return signals
