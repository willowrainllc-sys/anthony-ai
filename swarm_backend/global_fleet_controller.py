...
class GlobalFleetController:
    ...
    async def audit_global_fleet_health(self) -> GlobalFleetManifest:
        swarm_log("FLEET_CTRL: Running global mesh synchronization and health audit...", node="FLEET_CTRL")
        ...
        swarm_log(f" FLEET_CTRL SUCCESS: Mesh Healthy (Score: {manifest.mesh_health_score}%). All {len(nodes_status)} nodes synchronized.", node="FLEET_CTRL")
        return manifest
