...
class ObsidianReputationShield:
    ...
    async def run_reputation_lockdown_audit(self):
        swarm_log("REP_SHIELD: Performing high-precision reputation lockdown audit...", node="REP_SHIELD")

        audit = await rep_guardian.audit_matrix_reputation()
        score = audit.get("reputation_score", 100)

        if score < 95:
            swarm_log(f"[-] ALERT: IP Reputation dropped to {score}/100. Initiating EMERGENCY AUTO-ROTATION...", node="REP_SHIELD")
            ...
            swarm_log(" REP_SHIELD SUCCESS: Node matrix rotated to fresh IP block. Score restored.", node="REP_SHIELD")
            return {"status": "ROTATED", "score": 100}

        swarm_log(f" REP_SHIELD: Grid is secure. Score is steady at {score}/100.", node="REP_SHIELD")
