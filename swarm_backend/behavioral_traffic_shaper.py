...
class TrafficShaper:
    ...
    async def run_traffic_shaping_loop(self):
        swarm_log("SHAPER: Initiating human-like behavioral traffic shaping...", node="SHAPER")
        ...
        while self.active_shaping:
            try:
                ...
                swarm_log(f"SHAPER: Engaging high-velocity session for {session_duration}s", node="SHAPER")
                await asyncio.sleep(session_duration)

                # 2. Simulate "Reading/Pause" (Low noise)
                pause_duration = random.randint(600, 3600) # 10-60 mins
                swarm_log(f"SHAPER: Entering low-volume pause for {pause_duration}s", node="SHAPER")
                ...
                swarm_log(f"[-] SHAPER Note: {e}", node="SHAPER")
