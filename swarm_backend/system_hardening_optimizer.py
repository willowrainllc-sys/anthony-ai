...
class SystemHardeningOptimizer:
    ...
    def execute_docker_resource_isolation_check(self) -> dict:
        ...
        swarm_log("HARDENING: Enforcing Docker CPU/RAM isolation limits across rendering nodes...", node="HARDENING")
        return {
    ...
    def auto_sweep_onchain_treasury(self, current_usdc_balance: float = 1250.0) -> Optional[MultiSigTreasurySweep]:
        ...
        if current_usdc_balance >= self.sweep_threshold_usdc:
            ...
            swarm_log(f"HARDENING: Threshold cleared (${current_usdc_balance:.2f} >= ${self.sweep_threshold_usdc:.2f}). Sweeping USDC to Cold Storage...", node="HARDENING")
            ...
            swarm_log(f" HARDENING SUCCESS: Auto-swept ${current_usdc_balance:.2f} USDC to Multi-Sig Cold Storage [{self.cold_storage[:16]}...]!", node="HARDENING")
            return sweep
