...
class SquareRealBalanceMonitor:
    ...
    async def get_actual_bank_balance(self) -> dict:
        swarm_log(f"SQUARE_MONITOR: Auditing proposal sent funds for [{SQUARE_LOC}]...", node="SQUARE_MONITOR")
        ...
        try:
            async with httpx.AsyncClient(timeout=15.0) as client:
                ...
                if p_resp.status_code == 200:
                    ...
                    swarm_log(f"[-] SQUARE API ERROR: {p_resp.status_code}. Defaulting to PROJECTION.", node="SQUARE_MONITOR")
                    return self._get_projection()

        except Exception as e:
            swarm_log(f"[-] SQUARE EXCEPTION: {e}", node="SQUARE_MONITOR")
            return self._get_projection()
