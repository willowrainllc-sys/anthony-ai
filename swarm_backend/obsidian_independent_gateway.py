...
class ObsidianIndependentGateway:
    ...
    async def issue_direct_b2b_handshake(self, company_name: str, bandwidth_tb: float = 10.0) -> IndependentContract:
        swarm_log(f"GATEWAY: Handshaking with independent client [{company_name}]...", node="GATEWAY")
        ...
        swarm_log(f" GATEWAY SUCCESS: Direct handshake complete. Monthly Revenue Locked: ${fee:,.2f}", node="GATEWAY")
        return contract
