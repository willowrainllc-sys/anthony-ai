...
class OpenLineStreamEngine:
    ...
    def establish_open_line(self, client_identity: str) -> OpenLineStreamSession:
        ...
        swarm_log(f" OPEN_LINE SUCCESS: Established persistent socket stream for [{client_identity}] at {endpoint}!", node="OPEN_LINE")
        return session
    ...
    async def stream_chunk_telemetry(self, session: OpenLineStreamSession, chunk_mb: float = 250.0) -> dict:
        ...
        swarm_log(f"OPEN_LINE: Transferred {chunk_mb} MB across active line [{session.session_id}]. Total: {session.megabytes_transferred:.1f} MB", node="OPEN_LINE")
