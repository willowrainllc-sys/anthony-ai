...
class ObsidianCryptoExchanger:
    ...
    async def exchange_to_bitcoin(self, amount_usd: float, source_token: str = "JMPT") -> dict:
        swarm_log(f"EXCHANGER: Initiating swap for ${amount_usd:.2f} {source_token} -> BITCOIN...", node="FINANCE")
        ...
        swarm_log(f" EXCHANGER SUCCESS: Successfully converted {source_token} into {btc_qty} BTC. Dispatched to {self.target_wallet}.", node="FINANCE")
        return result
