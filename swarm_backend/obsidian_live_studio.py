...
class ObsidianLiveStudio:
    ...
    async def run_live_session(self):
        swarm_log(f"STUDIO: Launching Live View for [{TARGET_FILE.name}]...", node="SUPREME")
        ...
        async with async_playwright() as p:
            ...
            swarm_log("[*] STUDIO: Initial Ingress complete. Waiting for edits...", node="SUPREME")
            ...
            try:
                while True:
                    ...
                    if TARGET_FILE.exists():
                        ...
                        if current_mtime > self.last_mtime:
                            swarm_log(f"⚡ STUDIO: Change detected in {TARGET_FILE.name}. Reloading...", node="SUPREME")
                            await page.reload()
                ...
                swarm_log(f"[-] STUDIO ERROR: {e}", node="SUPREME")

            swarm_log("✓ STUDIO: Live View closed by Director.", node="SUPREME")
            await browser.close()
