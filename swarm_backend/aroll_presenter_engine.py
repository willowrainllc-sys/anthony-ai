...
class ArollPresenterEngine:
    ...
    async def get_aroll_presenter_clip(self, category: str = "documentary", width: int = 720, height: int = 1280) -> str:
        swarm_log(f"A-ROLL: Sniping primary A-Roll presenter host for [{category.upper()}]...", node="AROLL")
                        ...
                        if videos:
                            ...
                            if best_file:
                                ...
                                if out_path.exists() and out_path.stat().st_size > 300000:
                                    swarm_log(f" A-ROLL SUCCESS: Sniped primary presenter: {out_path.name}", node="AROLL")
                                    return str(out_path)
            except Exception as e:
                swarm_log(f"[-] A-Roll Snipe Note: {e}", node="AROLL")
