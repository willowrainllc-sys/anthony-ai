...
class Obsidian IngressLinker:
    ...
    async def link_account(self, auth_file: Path):
        ...
        swarm_log(f"LINKER: Processing [{account_name}] for ObsidianBridge binding...", node="SUPREME")
        ...
        async with async_playwright() as p:
            try:
                ...
                # 1. Access Dashboard
                swarm_log(f"   -> Accessing Obsidian Ingress for [{account_name}]...", node="SUPREME")
                ...
                if OBSIDIAN_BRIDGE_ID in content:
                    swarm_log(f"   [] ALREADY LINKED: [{account_name}] is already bound to ID.", node="SUPREME")
                    await browser.close()
                    return True
                ...
                if await submit_btn.count() > 0:
                    ...
                    swarm_log(f" LINKER SUCCESS: Account [{account_name}] is now feeding ObsidianBridge.", node="SUPREME")
                ...
                swarm_log(f"[-] LINKER ERROR: {e}", node="SUPREME")
                return False

    async def execute_mass_linking_strike(self, limit: int = 50):
        ...
        swarm_log(f"LINKER: Initiating mass strike on {len(hg_files)} accounts...", node="SUPREME")
...
if __name__ == "__main__":
    asyncio.run(linker.execute_mass_linking_strike())
