...
class ObsidianComputeRenter:
    ...
    async def execute_compute_yield_audit(self) -> float:
        ...
        swarm_log("COMPUTE_MGR: Auditing GPU/CPU Rental Yields...", node="COMPUTE_MGR")
        ...
        swarm_log(f" COMPUTE_MGR SUCCESS: Compute Active. Daily Yield: ${total_daily:,.2f} USD.", node="COMPUTE_MGR")
        return total_daily
    ...
    def generate_compute_manifest(self) -> ComputeRentalManifest:
        ...
        swarm_log(f" COMPUTE_MGR: Grand Compute Manifest [{self.manifest_id}] Locked.", node="COMPUTE_MGR")
        return manifest
