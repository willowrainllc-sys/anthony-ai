...
class GeonodeSalesSubmitBot:
    ...
    async def execute_sales_handshake(self, headless: bool = True) -> dict:
        swarm_log(f"GEONODE_SALES: Initiating Elite Handshake via Sales Portal (Headless={headless})...", node="GEONODE_BOT")
        ...
        async with async_playwright() as p:
            ...
            try:
                swarm_log(f"GEONODE_SALES: Navigating to {GEONODE_SALES_URL}...", node="GEONODE_BOT")
                ...
                try:
                    # Capture debug state
                    await page.screenshot(path="D:\\ObsidianAi_Swarm\\Temp\\geonode_sales_form_init.png")
                    ...
                    if await msg_input.is_visible():
                        ...
                        swarm_log(" GEONODE_SALES: Elite Proposal text injected.", node="GEONODE_BOT")

                    # Final Screenshot for confirmation
                    shot_path = f"D:\\ObsidianAi_Swarm\\Temp\\geonode_sales_ready_{uuid.uuid4().hex[:4]}.png"
                    ...
                    if await submit_btn.is_visible():
                        ...
                        swarm_log(" GEONODE_SALES SUCCESS: Handshake Proposal submitted to Geonode Sales!", node="GEONODE_BOT")
                    else:
                        swarm_log("[-] GEONODE_SALES: Submit button not found. Assuming manual completion or JS trigger.", node="GEONODE_BOT")
                    ...
                    swarm_log(f"[-] GEONODE_SALES Field Error: {e}", node="GEONODE_BOT")
                    await browser.close()
                    return {"status": "ERROR", "message": str(e)}

            except Exception as e:
                swarm_log(f"[-] GEONODE_SALES Navigation Error: {e}", node="GEONODE_BOT")
                await browser.close()
                return {"status": "ERROR", "message": str(e)}
