...
class ObsidianCaptchaSolver:
    ...
    async def solve_captcha_for_page(self, page, site_key: str = None) -> bool:
        swarm_log("CAPTCHA: Detecting puzzle signatures on target page...", node="CAPTCHA")

        # 1. Detection Logic
        # In a real run, this scans for <iframe> elements with captcha sources

        if not CAPTCHA_API_KEY:
            swarm_log("[-] CAPTCHA: No API key found. Attempting Local AI Vision solve...", node="CAPTCHA")
            # Logic to take screenshot of captcha and send to Llama 3.2 Vision
            return await self._local_vision_solve(page)

        swarm_log(" CAPTCHA: Dispatching puzzle to 2Captcha network...", node="CAPTCHA")
    ...
    async def _local_vision_solve(self, page) -> bool:
        ...
        try:
            screenshot_path = "D:\\ObsidianAi_Swarm\\Temp\\captcha_puzzle.png"
            ...
            swarm_log(f" CAPTCHA: Local AI Solution captured: [{solution}]", node="CAPTCHA")
            return True
        except:
            return False
