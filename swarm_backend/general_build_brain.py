...
# ============================================================
# 2. DOMAIN-SPECIFIC AGENT SWARM SELECTOR
# ============================================================
...
class UniversalBuildBrain:
    ...
    analyzes intent, selects the agent swarm, manages build_state, executes, tests,
    ...
    def create_project_build(self, user_goal: str) -> BuildState:
        ...
        swarm_agents = DOMAIN_SWARM_ROSTER.get(domain, DOMAIN_SWARM_ROSTER["code_fix"])

        swarm_log(f"BUILD_BRAIN: Analyzing objective [{user_goal[:35]}...] -> Domain: [{domain.upper()}]", node="BUILD_BRAIN")

        t1 = TaskItem(task_id="t1_research", description="Analyze requirements and local file context", assigned_agent=swarm_agents[0])
        t2 = TaskItem(task_id="t2_execute", description="Execute core build & code modifications", assigned_agent=swarm_agents[1] if len(swarm_agents) > 1 else swarm_agents[0])
        ...
        state = BuildState(
            ...
            agents_allocated=swarm_agents,
            tasks=[t1, t2, t3],
            decisions=[f"Allocated agent swarm: {', '.join(swarm_agents)}"],
            status="planning"
        )
        ...
        swarm_log(f" BUILD_BRAIN SUCCESS: Project [{project_id}] created for domain [{domain}]!", node="BUILD_BRAIN")
        return state
    ...
    def translate_to_human_explanation(self, state: BuildState) -> str:
        ...
        return (
            ...
            f" **Allocated Agent Swarm:** {', '.join(state.agents_allocated)}\n"
