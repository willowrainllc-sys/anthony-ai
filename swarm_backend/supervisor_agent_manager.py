...
class SupervisorManagerAgent:
    ...
    async def audit_action_before_execution(self, worker_name: str, action_type: str, payload: dict) -> dict:
        ...
        swarm_log(f"SUPERVISOR: Auditing worker [{worker_name}] action [{action_type}]...", node="SUPERVISOR")
        ...
        if approved:
            swarm_log(f" SUPERVISOR APPROVAL: [{worker_name} | {action_type}] Score: {score}/100.", node="SUPERVISOR")
        else:
            swarm_log(f"[-] SUPERVISOR REJECTION: [{worker_name} | {action_type}] Score: {score}/100. Reasons: {trace_record['reason']}", node="SUPERVISOR")
