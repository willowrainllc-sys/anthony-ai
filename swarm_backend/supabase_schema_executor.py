...
class SupabaseSchemaExecutor:
    ...
    async def execute_master_schema(self, headless: bool = True) -> dict:
        swarm_log(f"SUPABASE_SQL: Initiating Master Schema Execution (Headless={headless})...", node="SUPABASE_BOT")
        ...
        async with async_playwright() as p:
            ...
            try:
                swarm_log(f"SUPABASE_SQL: Navigating to SQL Editor...", node="SUPABASE_BOT")
                ...
                swarm_log("SUPABASE_SQL: Injecting SQL content into editor...", node="SUPABASE_BOT")
                ...
                swarm_log("SUPABASE_SQL: Triggering 'RUN' command...", node="SUPABASE_BOT")

                # Keyboard shortcut is more reliable for 'Run' (Ctrl + Enter)
                await page.keyboard.press("Control+Enter")

                swarm_log("SUPABASE_SQL: Waiting for execution results...", node="SUPABASE_BOT")
                await asyncio.sleep(10) # Wait for tables to provision

                # Capture confirmation
                shot_path = f"D:\\ObsidianAi_Swarm\\Temp\\supabase_execution_result_{uuid.uuid4().hex[:4]}.png"
                await page.screenshot(path=shot_path)

                swarm_log(f" SUPABASE_SQL SUCCESS: Master Schema Executed. Proof: {shot_path}", node="SUPABASE_BOT")

                await browser.close()
                return {"status": "SUCCESS", "message": "Master Schema executed successfully.", "preview": shot_path}

            except Exception as e:
                swarm_log(f"[-] SUPABASE_SQL Error: {e}", node="SUPABASE_BOT")
                await browser.close()
