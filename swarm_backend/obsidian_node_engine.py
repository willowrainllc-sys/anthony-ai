...
class ObsidianNodeEngine:
    ...
    def deploy_proxy_instance(self, protocol: str, port: int, rate_limit: float = 100.0) -> ProxyInstance:
        ...
        swarm_log(f" OBSIDIAN_NODE: Deployed {protocol} instance on port {port} (Limit: {rate_limit}Mbps)", node="NODE_ENGINE")
        return instance
    ...
    def generate_node_manifest(self) -> ObsidianNodeConfig:
        ...
        swarm_log(f" OBSIDIAN_NODE: Finalized manifest for [{self.node_id}] with {len(self.instances)} instances.", node="NODE_ENGINE")
        return manifest

    async def execute_node_handshake_pulse(self):
        """Simulates an industrial-grade node heartbeat pulse with telemetry logging."""
        swarm_log(f"NODE_ENGINE: Initiating handshake pulse for [{self.node_id}]...", node="NODE_ENGINE")
        ...
        swarm_log(f" OBSIDIAN_NODE SUCCESS: Pulse confirmed. Active Instances: {len(self.instances)}", node="NODE_ENGINE")
        return telemetry
