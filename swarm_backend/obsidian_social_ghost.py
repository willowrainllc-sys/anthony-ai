...
class ObsidianSocialGhost:
    ...
    async def execute_stealth_upload(self, platform, video_path, metadata):
        swarm_log(f"GHOST: Initiating uncatchable upload to [{platform}]...", node="MEDIA")
        ...
        swarm_log(f"GHOST: Holding for {wait_min}m to break upload pattern...", node="MEDIA")
    ...
    def _scrub_video_metadata(self, path):
        ...
        swarm_log(f" GHOST: Metadata incinerated for [{os.path.basename(path)}].", node="MEDIA")
