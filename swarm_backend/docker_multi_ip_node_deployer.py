...
class DockerMultiIpNodeDeployer:
    ...
    def generate_multi_ip_docker_compose(self, ip_addresses: list = None) -> dict:
        ...
        swarm_log(f"DOCKER_IP: Generating multi-IP Docker configuration across {len(ip_addresses)} distinct IPs...", node="DOCKER_IP")
        ...
        swarm_log(f" DOCKER_IP SUCCESS: Multi-IP Docker config generated at {out_file.name}!", node="DOCKER_IP")
        return {
            "status": "success",
            "ip_count": len(ip_addresses),
