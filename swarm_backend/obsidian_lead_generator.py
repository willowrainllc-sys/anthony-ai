...
class ObsidianLeadGenerator:
    ...
    async def harvest_high_intent_leads(self, category: str = "Real Estate") -> LeadPackage:
        swarm_log(f"LEAD_GEN: Harvesting high-intent leads for [{category}]...", node="LEAD_GEN")
        ...
        swarm_log(f" LEAD_GEN SUCCESS: Package [{package_id}] generated. Value: ${price:,.2f} USD.", node="LEAD_GEN")
        return package

    async def draft_outreach_email(self, lead: B2BLead) -> str:
        """Drafts a professional, high-aura outreach email for a specific lead."""
        swarm_log(f"LEAD_GEN: Drafting outreach for [{lead.company_name}]...", node="LEAD_GEN")
