...
class ObsidianGmailHarvester:
    ...
    def harvest_and_verify_all(self):
        swarm_log("HARVESTER: Initiating direct IMAP verification strike...", node="SECURITY")

        if not GMAIL_PASS:
            swarm_log("[-] HARVESTER: No GMAIL_APP_PASSWORD found.", node="SECURITY")
            return 0
        ...
        try:
            with imapclient.IMAPClient('imap.gmail.com', use_uid=True) as client:
                ...
                swarm_log(f"HARVESTER: Detected {len(messages)} unread verification signals.", node="SECURITY")
                ...
                for msg_id, data in client.fetch(messages, ['RFC822']).items():
                    ...
                    if links:
                        verify_url = links[0]
                        swarm_log(f" HARVESTER: Sniped link for verify. Handshaking...", node="SECURITY")
            ...
            swarm_log(f"[-] HARVESTER ERROR: {e}", node="SECURITY")
            return 0
