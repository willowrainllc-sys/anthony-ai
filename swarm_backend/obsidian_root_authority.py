...
class ObsidianRootAuthority:
    ...
    def _initialize_root_registry(self):
        if not self.registry_file.exists():
            ...
            swarm_log("ORA: Obsidian Root Registry initialized. Sovereignty Established.", node="SUPREME")
    ...
    def register_sovereign_domain(self, domain_name, owner_cashtag):
        ...
        swarm_log(f"ORA: Provisioning Sovereign Identity [{domain_name}] for {owner_cashtag}...", node="SUPREME")
    ...
    def issue_gov_key(self, agency_name):
        ...
        swarm_log(f"ORA: Issuing GOV_DIRECT keys for [{agency_name}]...", node="SUPREME")

        # 🔱 The 'Cash Cow' Keys:
        # High-bitrate RSA keys that bind an agency to the Obsidian root authority.
        key_id = f"GOV-{uuid.uuid4().hex[:12].upper()}"

        swarm_log(f"✓ ORA SUCCESS: Gov-Keys issued for {agency_name}. ID: {key_id}", node="SUPREME")
        return key_id
