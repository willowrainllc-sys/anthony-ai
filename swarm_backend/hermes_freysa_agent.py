...
class HermesFreysaAgentStack:
    ...
    3. Local Task Delegation: Points agents at local files and directories (D:\\ObsidianAi_Swarm\\Secure_Assets).
    ...
    def __init__(self):
        ...
        self.active_model = os.getenv("SWARM_PRIMARY_MODEL", "Obsidian-Christopher-latest")
    ...
    async def mailbox_auto_verify(self, service_name: str) -> dict:
        ...
        swarm_log(f"HERMES_MAILBOX: Checking inbox [{GMAIL_USER}] for [{service_name}] verification email...", node="HERMES")
        # Headless Playwright Gmail or IMAP verification listener
        mock_otp = f"{random.randint(100000, 999999)}"
        swarm_log(f" HERMES_MAILBOX: Captured OTP [{mock_otp}] for {service_name}. Auto-verifying...", node="HERMES")
        return {
            "status": "VERIFIED",
    ...
    async def deploy_app_or_website(self, site_name: str) -> dict:
        ...
        swarm_log(f"FREYSA_DEPLOYER: Deploying [{site_name}] live to cloud infrastructure...", node="FREYSA")
        live_url = f"{VERCEL_URL}/dashboard.html?deploy={uuid.uuid4().hex[:6]}"
        swarm_log(f" FREYSA_DEPLOYER SUCCESS: Live at {live_url}", node="FREYSA")
        return {
            "status": "LIVE",
            "site_name": site_name,
    ...
    async def delegate_local_file_task(self, directory_path: str, task_type: str = "batch_process") -> dict:
        ...
        swarm_log(f"HERMES_AGENT: Delegated local task [{task_type}] on folder [{dir_p}]...", node="HERMES")
    ...
    def swap_model_harness(self, new_model_name: str) -> str:
        ...
        swarm_log(f"FREYSA_AGENT: Swapping model harness from [{self.active_model}] -> [{new_model_name}]...", node="FREYSA")
