...
class ObsidianCarrierWipe:
    ...
    def execute_full_wipe(self):
        swarm_log(f"[IMPERIUM] WIPE: Initiating full carrier identity incineration on [{self.serial}]...", node="CARRIER")
        ...
        for carrier in carriers_to_delete:
            swarm_log(f"WIPE: Deleting legacy {carrier} certificates...", node="CARRIER")
            # [EXECUTE] adb -s {self.serial} shell rm -rf /system/etc/security/cacerts/{carrier}* [/EXECUTE]

        # 3. Inject Obsidian Branding
        swarm_log("WIPE: Injecting [SUPREME] OBSIDIAN GLOBAL branding into system UI...", node="CARRIER")
        # [EXECUTE] adb -s {self.serial} shell settings put global multi_sim_carrier_name '[SUPREME] OBSIDIAN GLOBAL' [/EXECUTE]

        swarm_log(f" WIPE SUCCESS: Device [{self.serial}] is now an OBSIDIAN AUTHORITY node.", node="CARRIER")
