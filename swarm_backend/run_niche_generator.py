...
async def build_random_niche_masterpiece():
    ...
    swarm_log(f"1. STARTING AUTONOMOUS NICHE PIPELINE [{job_id}]...", node="NICHE_GEN")

    # Step 1: Generate 3-Minute Production Story Plan
    blueprint = story_director_v3.generate_director_blueprint()
    swarm_log(f"   [] Topic: {blueprint.topic}", node="NICHE_GEN")
    swarm_log(f"   [] Title: {blueprint.title}", node="NICHE_GEN")
    ...
    swarm_log(f"2. RESOLVING VISUAL TIMELINE VIA VISUAL DIRECTOR V4.2...", node="NICHE_GEN")
    ...
    swarm_log(f"3. RENDERING 3-MINUTE CINEMATIC VIDEO (niche_director_cut_live.mp4)...", node="NICHE_GEN")
    ...
    if not res or not res.get("output_path"):
        swarm_log("[-] NICHE PIPELINE FAIL: Video rendering returned no output.", node="NICHE_GEN")
    ...
    if PUBLISH_LIVE_ENV:
        swarm_log("4. PUBLISHING LIVE TO YOUTUBE...", node="NICHE_GEN")
        ...
        swarm_log(f"=== LIVE YOUTUBE 3-MIN NICHE RESULT: {yt_url} ===", node="NICHE_GEN")
        return {"status": "published", "title": blueprint.title, "youtube_url": yt_url, "video_path": v_path}
    else:
        swarm_log(f" TEST MODE (PUBLISH_LIVE=false): Video rendered locally at {v_path}", node="NICHE_GEN")
