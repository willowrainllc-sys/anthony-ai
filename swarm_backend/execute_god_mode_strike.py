...
async def main():
    print("--- Initiating Obsidian God-Mode Strike ---")

    # 1. Trigger 5k Aggregator Strike
    print("\n[1/4] Triggering 5k Aggregator Strike for SpaceX_Data_Logistics...")
    await aggregator_core.synchronize_virtual_swarm()
    strike_res = await aggregator_core.generate_b2b_wholesale_invoice(
        client_name='SpaceX_Data_Logistics',
        volume_gb=865.0
    )
    print(f"Result: {json.dumps(strike_res, indent=2)}")

    # 2. Trigger Master Strike
