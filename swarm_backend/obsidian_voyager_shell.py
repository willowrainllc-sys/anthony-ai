...
class ObsidianObsidian TitanShell:
    ...
    """
    def __init__(self):
        self.user_data_dir = Path(r"D:\AnthonyAi_Swarm\Secure_Assets\Obsidian Titan_Session")
        self.user_data_dir.mkdir(parents=True, exist_ok=True)

    async def launch_voyager(self, start_url="http://127.0.0.1:80", node_id="DIRECTOR"):
        swarm_log(f"🛰️ OBSIDIAN_TITAN: Igniting Sovereign Browser Shell for [{node_id}]...", node="SUPREME")
        ...
        async with async_playwright() as p:
            ...
            try:
                swarm_log(f"[*] OBSIDIAN_TITAN: Navigating to {start_url}...", node="SUPREME")
                await page.goto(start_url, timeout=10000)
            except:
                swarm_log("[-] OBSIDIAN_TITAN: External domain unresolved. Falling back to Local Portal.", node="SUPREME")
                await page.goto("http://127.0.0.1:80")

            swarm_log(f"✓ OBSIDIAN_TITAN SUCCESS: Ingress established on [{node_id}].", node="SUPREME")
