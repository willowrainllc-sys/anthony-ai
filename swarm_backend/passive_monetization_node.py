...
class PassiveMonetizationNode:
    ...
    async def get_obsidian_ingress_sdk_stats(self) -> dict:
        if self.hg_client:
            try:
                ...
                swarm_log(f"[-] Obsidian Ingress SDK Note: {e}", node="PASSIVE_MONEY")
    ...
    async def get_passive_earnings_summary(self) -> dict:
        swarm_log("MONETIZATION: Syncing DePIN & Proxy Sharing Networks (Obsidian Ingress, EarnApp, Repocket, Grass AI, Mysterium)...", node="PASSIVE_MONEY")
        now = time.time()
