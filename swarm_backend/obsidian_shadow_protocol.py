...
class ObsidianShadowProtocol:
    ...
    async def execute_traffic_masking(self, page):
        ...
        swarm_log("SHADOW: Injecting organic traffic noise...", node="SECURITY")
        try:
            ...
            swarm_log(f" SHADOW: Masking successful on [{target}].", node="SECURITY")
        except: pass

    async def apply_stealth_jitter(self):
        """Breaks the predictable 60-second heartbeat cycle."""
        delay = random.randint(10, 180)
        swarm_log(f"SHADOW: Breaking cycle pattern with {delay}s jitter...", node="SECURITY")
