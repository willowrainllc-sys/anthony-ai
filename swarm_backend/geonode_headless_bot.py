...
class GeonodeHeadlessBot:
    ...
    async def run_geonode_submission(self, headless: bool = True) -> dict:
        swarm_log(f"GEONODE_BOT: Initializing Headless Session (Headless={headless})...", node="GEONODE_BOT")
        ...
        async with async_playwright() as p:
            ...
            if vaulted_state:
                swarm_log("GEONODE_BOT: Injecting vaulted session cookies...", node="GEONODE_BOT")
                await context.add_cookies(vaulted_state.get("cookies", []))

            page = await context.new_page()

            try:
                swarm_log(f"GEONODE_BOT: Navigating to {GEONODE_PROVIDER_URL}...", node="GEONODE_BOT")
                ...
                swarm_log(f" GEONODE_BOT: Connected to [{title}] at {url}", node="GEONODE_BOT")
                ...
                swarm_log(f"[-] GEONODE_BOT Exception: {e}", node="GEONODE_BOT")
                await browser.close()
                return {"status": "ERROR", "message": str(e)}

    async def run_interactive_login(self):
        """Launches a visible browser for manual Geonode login/session capture."""
        swarm_log("GEONODE_BOT: Launching interactive session. Please log in or reach the provider page.", node="GEONODE_BOT")
        ...
        async with async_playwright() as p:
            ...
            swarm_log("GEONODE_BOT: Interactive session closed.", node="GEONODE_BOT")
