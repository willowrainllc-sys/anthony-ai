...
class GmailCodeHarvester:
    ...
    async def harvest_codes_from_inbox(self, headless: bool = True) -> int:
        swarm_log(f"HARVESTER: Initiating real-world code extraction from Gmail (Headless={headless})...", node="HARVESTER")

        vaulted_state = cookie_monster.get_vaulted_cookies("gmail")
        if not vaulted_state:
            swarm_log("[-] HARVESTER: Gmail session cookies not found.", node="HARVESTER")
            return 0
        ...
        async with async_playwright() as p:
            ...
            try:
                swarm_log("HARVESTER: Navigating to filtered inbox...", node="HARVESTER")
                await page.goto(GMAIL_SEARCH_URL, timeout=90000, wait_until="networkidle")
                await asyncio.sleep(8)

                # Capture search results for audit
                await page.screenshot(path="D:\\ObsidianAi_Swarm\\Temp\\gmail_harvest_scan.png")

                # 1. Get all unread email rows
                email_rows = page.locator('tr.zA').all()
                count = await email_rows
                swarm_log(f"HARVESTER: Found {len(count)} potential reward emails.", node="HARVESTER")
                ...
                swarm_log(f" HARVESTER SUCCESS: Locked {codes_found} new codes into the Obsidian Vault.", node="HARVESTER")
                return codes_found

            except Exception as e:
                swarm_log(f"[-] HARVESTER ERROR: {e}", node="HARVESTER")
                return 0
            finally:
                await browser.close()
