...
class ObsidianArbitrageEngine:
    ...
    async def scout_domain_arbitrage(self) -> ArbitrageOpportunity:
        ...
        swarm_log(f"ARBITRAGE: Analyzing domain potential for [{spark['subject']}]...", node="ARBITRAGE")
        ...
        swarm_log(f" ARBITRAGE SUCCESS: Opportunity [{opp_id}] Locked: {opp.asset_name} (Est. Value: ${opp.est_exit_value_usd:,.2f})", node="ARBITRAGE")
    ...
    async def execute_arbitrage_pulse(self):
        ...
        swarm_log("ARBITRAGE: Initiating systematic digital asset scan...", node="ARBITRAGE")
