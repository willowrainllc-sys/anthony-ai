...
class WholesaleProxyResellerGateway:
    ...
    async def generate_reseller_checkout(self, tier_key: str = "corporate_standard") -> dict:
        ...
        swarm_log(f"RESELLER_GATEWAY: Generating Square B2B link for [{tier['name']}] (${tier['price_usd']}/mo)...", node="RESELLER")
    ...
    async def provision_wholesale_proxy_port(self, company_name: str, tier_key: str = "corporate_standard") -> ProvisionedClientPort:
        ...
        swarm_log(f" RESELLER_GATEWAY SUCCESS: Provisioned [{tier['name']}] port {host_ip}:{port} for {company_name}!", node="RESELLER")
        return port_obj
    ...
    def record_usage_and_check_cutoff(self, client_id: str, bytes_transferred: int) -> dict:
        ...
        with db._get_connection() as conn:
            ...
            if row:
                ...
                if is_active == 0:
                    swarm_log(f"[-] RESELLER_CUTOFF: Client [{client_id}] reached quota ({current_used:.1f}/{quota:.1f} GB). Port locked until renewal.", node="RESELLER")
