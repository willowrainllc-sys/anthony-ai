...
class YoutubeTrendingHashtagScraper:
    ...
    async def scrape_live_trending_hashtags(self, subject_query: str, limit: int = 8) -> list:
        swarm_log(f"YOUTUBE_SCRAPER: Scraping live trending hashtags for [{subject_query}]...", node="YT_SCRAPER")
        ...
        if self.api_key:
            try:
                ...
                swarm_log(f"[-] YouTube Scraper Note: {e}", node="YT_SCRAPER")
        ...
        swarm_log(f" YOUTUBE_SCRAPER SUCCESS: Scraped {len(tag_list)} live tags -> {' '.join(tag_list)}", node="YT_SCRAPER")
        return tag_list
