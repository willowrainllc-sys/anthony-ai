...
class PlaywrightHumanTrainer:
    ...
    2. SOCIAL STRIKE: Handles video uploads to X, FB, and YouTube via the real web UI.
    ...
    async def train_gmail_verify(self, session_file: Path):
        swarm_log(f"TRAINER: Training bot on Gmail verification -> {session_file.name}", node="SECURITY")
        ...
        async with async_playwright() as p:
            ...
            try:
                ...
                if await email_row.is_visible():
                    ...
                    swarm_log(" TRAINER SUCCESS: Gmail handshake verified via Playwright.", node="SECURITY")

                await browser.close()
            except Exception as e:
                swarm_log(f"[-] TRAINER ERROR: {e}", node="SECURITY")
                await browser.close()
