...
class ObsidianEmergencyNotifier:
    ...
    def send_failover_alert(self, new_ip: str, new_port: int):
        swarm_log(f" EMERGENCY: Sending failover alert to Director Maestas...", node="SECURITY")

        if not GMAIL_PASS:
            swarm_log("[-] ALERT FAIL: No GMAIL_APP_PASSWORD found in .env.", node="SECURITY")
            return False

        subject = "[SUPREME] WILLOW RAIN SECURITY: GRID FAILOVER ACTIVATED"
        command_text = f"python swarm_backend/obsidian_production_launcher.py --remote {new_ip}:{new_port}"
        ...
        try:
            ...
            swarm_log(" EMERGENCY: Failover alert dispatched to Gmail.", node="SECURITY")
            return True
        except Exception as e:
            swarm_log(f"[-] ALERT ERROR: {e}", node="SECURITY")
            return False
