...
class RobinhoodBrowserBot:
    ...
    async def _handle_interruptions(self, page, snapshot_dir: Path) -> str:
        ...
        if "verification code" in content_lower or "two-factor authentication" in content_lower or "enter the code" in content_lower:
            swarm_log("[-] ROBINHOOD 2FA WALL DETECTED. Manual cookie refresh required.", node="ROBINHOOD")
        ...
        for text in dismiss_buttons:
            try:
                ...
                if await btn.is_visible(timeout=1500):
                    swarm_log(f"ROBINHOOD: Bypassing splash screen / modal by clicking '{text}'...", node="ROBINHOOD")
                    await btn.click()
    ...
    async def get_real_status(self) -> dict:
        swarm_log("ROBINHOOD: Executing physical balance audit...", node="ROBINHOOD")
    ...
    async def execute_real_trade(self, symbol: str, action: str, amount_usd: float, apply_stop_loss: bool = True, stop_loss_pct: float = 0.05) -> dict:
        swarm_log(f"ROBINHOOD: Initiating PHYSICAL {action} on [{symbol}] for ${amount_usd}...", node="ROBINHOOD")
        ...
        async with async_playwright() as p:
            ...
            try:
                ...
                if current_price > 0 and apply_stop_loss and action.upper() == "BUY":
                    ...
                    swarm_log(f"ROBINHOOD: Market Price ${current_price}. Calculating Stop-Loss at ${stop_price:.2f}", node="ROBINHOOD")
                    ...
                    if await order_dropdown.count() > 0:
                        ...
                        if await stop_option.count() > 0:
                            ...
                            if await stop_input.count() > 0:
                                ...
                                await asyncio.sleep(1)
                                swarm_log("ROBINHOOD: Stop Loss attached successfully to form.", node="ROBINHOOD")
                ...
                if await review_btn.is_enabled():
                    ...
                    if await submit_btn.is_visible():
                        ...
                        swarm_log(f"[SUPREME] SUCCESS: Physical {action} on [{symbol}] DISPATCHED with Stop-Loss.", node="ROBINHOOD")
                ...
                swarm_log(f"[-] TRADE FAIL: Submission button not clickable for [{symbol}].", node="ROBINHOOD")
                ...
                swarm_log(f"[-] ROBINHOOD ERROR: {e}", node="ROBINHOOD")
