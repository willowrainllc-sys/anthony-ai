...
class TrafficOperatorBroker:
    ...
    async def run_traffic_operator_cycle(self) -> dict:
        swarm_log(f"TRAFFIC_OPERATOR: Routing priority relay traffic across {len(self.subnets)} registered ports...", node="TRAFFIC_OP")
        ...
        swarm_log(f" TRAFFIC_OPERATOR SUCCESS: Routed {total_gb_routed} GB across 16 ports -> Earned ${total_earned_usd} USD auto-deposited to Square!", node="TRAFFIC_OP")
