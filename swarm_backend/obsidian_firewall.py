...
class ObsidianFirewall:
    ...
    The "Red-Line" Protocol. Protects the Director's Main Accounts from the Swarm.
    ...
    2. IP AIR-GAPPING: The Master Console routes through a dedicated 'Authority Port' (8000)
       which is never used by the bot-swarm (Ports 1080-2080).
    ...
    def enforce_isolation(self, target_node_id: str, proxy_url: str):
        ...
        if self.master_ip in proxy_url:
            swarm_log(f"[ALERT] FIREWALL: CRITICAL COLLISION! Node [{target_node_id}] attempted to use Master IP. BLOCKED.", node="SECURITY")
            return False

        swarm_log(f" FIREWALL: Node [{target_node_id}] verified as air-gapped from Primary Authority.", node="SECURITY")
        return True
