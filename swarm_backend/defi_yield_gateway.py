...
class DefiYieldGateway:
    ...
    async def deploy_treasury_to_yield(self, amount: float = 500.0) -> StakingPosition:
        ...
        swarm_log(f"DeFi_GATEWAY: Deploying ${amount:.2f} {best_strategy['asset']} to [{best_strategy['protocol']}] at {best_strategy['apy_pct']}% APY...", node="DeFi_YIELD")
        ...
        swarm_log(f" DeFi_GATEWAY SUCCESS: Position [{staking_id}] active on {best_strategy['network']}. Projected Yield: ${position.projected_annual_yield}/yr", node="DeFi_YIELD")
        return position
