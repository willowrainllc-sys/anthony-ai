...
class ObsidianTreasuryManager:
    ...
    3. TEAM DISTRIBUTION: Manages the 30% 'Swarm' commission pool.
    ...
    async def allocate_incoming_revenue(self, amount_usd: float, source: str = "GENERAL"):
        ...
        swarm_log(f"TREASURY: Allocating ${amount_usd:,.2f} from [{source}]...", node="TREASURY")
        ...
        swarm_log(f" TREASURY SUCCESS: Revenue split complete. Director Draw: +${splits['DIRECTOR_DRAW']:,.2f}", node="TREASURY")
