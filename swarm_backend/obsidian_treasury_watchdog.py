...
class ObsidianTreasuryWatchdog:
    ...
    async def run_settlement_monitor(self):
        swarm_log("[IMPERIUM] TREASURY: Settlement Watchdog is ONLINE and hunting for payments...", node="FINANCE")
            ...
            try:
                ...
                for p in payments:
                    ...
                    if amount >= 100.0: # Only track high-aura payments
                        swarm_log(f"[SUPREME] TREASURY: New Settlement Detected! ${amount:,.2f} from {p.get('source_type')}", node="FINANCE")
                ...
                swarm_log(f"[-] TREASURY ERROR: {e}", node="FINANCE")
                await asyncio.sleep(60)

    def _trigger_wealth_alert(self, amount):
        """Pushes the 'Victory' signal to the Director's HUD."""
        swarm_log(f"[STRIKE] MISSION SUCCESS: ${amount:,.2f} HAS LANDED. Wealth built.", node="FINANCE")
