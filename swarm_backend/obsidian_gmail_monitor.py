...
class ObsidianGmailMonitor:
    ...
    async def run_verification_sweep(self):
        swarm_log("GMAIL_MONITOR: Initiating account verification sweep...", node="SECURITY")

        if not SESSION_FILE.exists():
            swarm_log("[-] GMAIL_MONITOR: No session keys found.", node="SECURITY")
            return False
        ...
        async with async_playwright() as p:
            ...
            try:
                ...
                swarm_log("GMAIL_MONITOR: Searching for 'Confirm your email' signals...", node="SECURITY")
                ...
                swarm_log(f"GMAIL_MONITOR: Found {count} unread verification signals.", node="SECURITY")
                ...
                for i in range(count):
                    ...
                    if await verify_link.count() > 0:
                        ...
                        swarm_log(f" GMAIL_MONITOR: Verification link sniped: {url[:50]}...", node="SECURITY")
                ...
                swarm_log(f"[-] GMAIL_MONITOR ERROR: {e}", node="SECURITY")
                await browser.close()
                return False
