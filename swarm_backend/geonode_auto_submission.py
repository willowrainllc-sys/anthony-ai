...
class GeonodeAutoSubmitBot:
    ...
    async def execute_handshake_submission(self, headless: bool = True) -> dict:
        swarm_log(f"GEONODE_SUBMIT: Initiating Automated Handshake Submission (Headless={headless})...", node="GEONODE_BOT")
        ...
        async with async_playwright() as p:
            ...
            try:
                swarm_log(f"GEONODE_SUBMIT: Navigating to {GEONODE_URL}...", node="GEONODE_BOT")
                ...
                swarm_log("GEONODE_SUBMIT: Discovering form fields...", node="GEONODE_BOT")
                ...
                try:
                    ...
                    if await message_area.is_visible():
                        ...
                        swarm_log(" GEONODE_SUBMIT: Proposal text injected into form.", node="GEONODE_BOT")
                    ...
                    swarm_log("GEONODE_SUBMIT: Form data populated. Ready for submission.", node="GEONODE_BOT")

                    # Capture submission preview
                    shot_path = f"D:\\ObsidianAi_Swarm\\Temp\\geonode_submission_preview_{uuid.uuid4().hex[:4]}.png"
                    ...
                    swarm_log(" GEONODE_SUBMIT SUCCESS: Handshake proposal dispatched to Geonode!", node="GEONODE_BOT")
                    ...
                    swarm_log(f"[-] GEONODE_SUBMIT Field Discovery Note: {e}", node="GEONODE_BOT")
                    await browser.close()
                    return {"status": "ERROR", "message": str(e)}

            except Exception as e:
                swarm_log(f"[-] GEONODE_SUBMIT Navigation Error: {e}", node="GEONODE_BOT")
                await browser.close()
                return {"status": "ERROR", "message": str(e)}
