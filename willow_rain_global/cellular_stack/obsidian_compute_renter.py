...
class ObsidianComputeRenter:
    ...
    1. RESOURCE AUDIT: Detects available cores and RAM across the 103 nodes.
    ...
    async def start_compute_lease(self):
        swarm_log("[IMPERIUM] COMPUTE: Initiating industrial compute rental strike...", node="CARRIER")
        ...
        while self.is_active:
            try:
                ...
                if cpu_usage < 70:
                    swarm_log(f" COMPUTE: Capacity found. Leasing {leasable_cores} cores to the Grid.", node="CARRIER")
                    ...
                    swarm_log("[ALERT] COMPUTE: High local load. Throttling rental...", node="CARRIER")

                await asyncio.sleep(600) # Audit every 10 mins

            except Exception as e:
                swarm_log(f"[-] COMPUTE ERROR: {e}", node="CARRIER")
