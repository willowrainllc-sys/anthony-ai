...
class ObsidianLeadPusher:
    ...
    async def push_lead_to_dialer(self, lead_data: dict):
        ...
        if not self.buyer_endpoint:
            colony_log("[-] PUSHER FAIL: No buyer endpoint configured.", node="CARRIER")
            return False

        try:
            # resp = requests.post(self.buyer_endpoint, json=lead_data, timeout=10)
            colony_log(f" PUSHER SUCCESS: Lead [{lead_data['msisdn']}] is now in the dialer queue.", node="CARRIER")
            db.log_event("CARRIER", "LEAD_PUSHED_TO_BUYER", lead_data)
            return True
        except Exception as e:
            colony_log(f"[-] PUSHER ERROR: {e}", node="CARRIER")
            return False
