...
class ObsidianAlchemicalTransmutation:
    ...
    async def execute_transmutation_burst(self):
        colony_log("[ATOMIC] ALCHEMY: Initiating Source Ascension... Turning Base to Gold.", node="POWER")

        while self.is_active:
            try:
                # 1. Capture the 'Base' (Ambient RF from the The Nest)
                ...
                colony_log(f"⚜️ ALCHEMY: Transmuted {raw_hz:.2f}Hz into ${gold_yield_usd:.2f} Liquid Gold.", node="POWER")
