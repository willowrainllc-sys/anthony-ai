...
class ObsidianOpSecAudit:
    ...
    async def run_security_sweep(self):
        swarm_log(f"🛡️ OPSEC: Initiating public footprint sweep for [{self.director_name}]...", node="SECURITY")

        for finding in self.findings:
            swarm_log(f"  [!] EXPOSURE DETECTED: {finding['type']} - {finding.get('platform') or finding.get('address') or finding.get('entity')}", node="SECURITY")
            db.log_event("SECURITY", "OPSEC_EXPOSURE", finding)

        swarm_log(f"✓ OPSEC SUCCESS: 3 Critical exposure points identified.", node="SECURITY")
