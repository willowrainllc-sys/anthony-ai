...
DNA_ENV = os.getenv("DNA_ENV", "test")
...
BASE_URL = DNA_URLS.get(DNA_ENV, "https://rest-test.domainnameapi.com")
...
class ObsidianDnaBridge:
    ...
    def __init__(self):
        ...
        auth_str = f"{DNA_RESELLER_ID}:{DNA_API_KEY}"
    ...
    async def check_availability(self, domain: str):
        ...
        try:
            ...
            data = resp.json()
            # Handle DNA specific response format
...
if __name__ == "__main__":
    async def test():
        ...
        res = await dna_bridge.check_availability("obsidian-test-node.com")
