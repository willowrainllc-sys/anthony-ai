/* [+] OBSIDIAN PERFORMANCE ENGINE v2.0 (PRODUCTION) */
...
function initObsidianAIChat() {
    ...
    // [+] Premium "Admin" Thumbnail Mascot
    ...
    win.innerHTML = `
        ...
        <div class="chat-body" id="obsidian_aiChatBody">
            <div class="msg msg-obsidian_ai">Greetings. I am Obsidian AI, your Business Intelligence (AI). How can I assist your business growth today?</div>
        </div>
...
/**
 * [+] PERFORMANCE HINTS: Tooltip system for key features
 */
const PERFORMANCE_HINTS = {
    "sharingToggle": "Director's Note: Every GB shared strengthens the Global Network and earns you direct credits.",
    "custEmail": "Security First: We use PQC encryption to deliver your assets to this verified inbox."
};

function initHints() {
    const card = document.createElement('div');
    card.className = 'performance-hint-card glass';
    card.id = 'performanceHintCard';
    document.body.appendChild(card);

    Object.keys(PERFORMANCE_HINTS).forEach(id => {
        const el = document.getElementById(id);
        if (el) {
            const btn = document.createElement('div');
            btn.className = 'performance-hint-btn';
            btn.innerText = '?';
            ...
            btn.onmouseenter = () => {
                card.innerHTML = `
                    ...
                    <p class="text-white text-xs font-medium leading-relaxed">${PERFORMANCE_HINTS[id]}</p>
                `;
...
async function handleChatKey(e) {
    if (e.key === 'Enter') {
        ...
        try {
            ...
            appendMessage('obsidian_ai', "The system is currently busy. Please ensure the Private Server is running or contact tech support.");
        }
    }
}
...
function injectMonetization() {
    ...
    if (sections.length > 2) {
        ...
        adContainer.className = 'obsidian-ad-banner performance-fade-up';
        ...
        adContainer.innerHTML = `
            ...
            <div id="obsidianAdFallback" class="flex items-center gap-10 px-8 w-full h-full cursor-pointer" onclick="window.location.href='obsidian_llc_formation.html'">
                ...
                <div>
                    <div class="text-[10px] font-black text-blue-400 uppercase tracking-widest">Enterprise Partner Offer</div>
            ...
            <ins class="adsbygoogle" style="display:none" data-ad-client="ca-pub-9539640812310468" data-ad-slot="AUTO_GENERATED" data-ad-format="auto" data-full-width-responsive="true"></ins>
        `;
        ...
        try {
            ...
            console.warn("[PERFORMANCE] AdSense connection delayed. Professional Fallback Active.");
        }
    }
}
 ...
 */
async function initPerformanceVideo() {
    document.body.style.backgroundColor = 'transparent';
    const container = document.createElement('div');
    container.className = 'performance-video-container';

    const video = document.createElement('video');
    video.className = 'performance-video-element';
    video.autoplay = true;
    video.muted = true;
    video.loop = true;
    ...
    overlay.className = 'performance-video-overlay';
    ...
    try {
        ...
        const resp = await fetch(getApiUrl(`/api/performance/video?query=${encodeURIComponent(page + ' people office tech blue')}`));
        const data = await resp.json();
        if (data.success) {
...
const SECURITY_TIPS = [
    ...
    "Fact: Public Wi-Fi is a major access point for sniffers. Use the Obsidian Network.",
...
function spawnSecurityPulse() {
    ...
    pulse.className = 'performance-fade-up glass p-6 rounded-2xl fixed bottom-32 left-8 z-[1000000] max-w-xs shadow-2xl';
...
function spawnConnectionPulse() {
    ...
    const connections = [
        ...
        { name: "Discord", url: "https://discord.gg/obsidiancity", icon: "💬", sub: "Join the Network" }
    ];

    const conn = connections[Math.floor(Math.random() * connections.length)];
    const pulse = document.createElement('div');
    pulse.className = 'performance-fade-up glass p-6 rounded-2xl fixed bottom-32 right-8 z-[1000000] max-w-xs shadow-2xl cursor-pointer';
...
function spawnPerformanceAd() {
    if (Math.random() > 0.4) return;
    const offer = PERFORMANCE_OFFERS[Math.floor(Math.random() * PERFORMANCE_OFFERS.length)];
    const ad = document.createElement('div');
    ad.className = 'performance-ad-icon';
    ad.style.left = `${Math.random() * (window.innerWidth - 150) + 75}px`;
    ad.style.top = `${Math.random() * (window.innerHeight - 300) + 150}px`;
    ad.innerHTML = `
        <img src="${offer.img}" class="performance-ad-img">
        <div class="performance-ad-info">
            <span class="performance-ad-label-sub">${offer.label}</span>
            <span class="performance-ad-price">${offer.price}</span>
        </div>
    `;
...
function handleScrollAnimations() {
    ...
    document.querySelectorAll('.performance-fade-up, .base44-card').forEach(el => {
        el.classList.add('performance-fade-up');
...
function injectNewsTicker() {
    ...
    const newsItems = [
        "Obsidian AI builds 1,000th professional node this hour",
        "St. Charles Global HQ confirms 99.9% uptime for Edge Network",
        ".COM wholesale prices holding steady at $14.70",
        "New LLC formation protocol reduces state filing time by 40%",
        "Integration engine neutralizes connection issues"
    ];

    let itemsHtml = '';
    newsItems.forEach(item => {
        itemsHtml += `<div class="news-item"><span class="news-tag">UPDATE</span> ${item}</div>`;
    });
...
/**
 * [+] CONTEXTUAL LINK ENGINE (Skimlinks Hybrid)
...
function injectSponsoredContent() {
    ...
    grid.className = 'obsidian-sponsored-grid performance-fade-up';
    ...
    const partners = [
        ...
        { title: "Get Industrial Ad Access with Ad Campaign", label: "Partner Offer", img: "https://images.unsplash.com/photo-1460925895917-afdab827c52f?q=80&w=400" },
        { title: "The Private Guide to Post-Quantum Identity", label: "Professional Reading", img: "https://images.unsplash.com/photo-1550751827-4bd374c3f58b?q=80&w=400" }
    ];
    ...
    grid.innerHTML = `
        <div class="w-full mb-10">
            <h3 class="text-xl font-black text-slate-900 tracking-tighter uppercase">Recommended for Your Business</h3>
        </div>
        ${cardsHtml}
    `;
    footer.before(grid);
...
window.addEventListener('DOMContentLoaded', () => {
    if (window.OBSIDIAN_PERFORMANCE_PULSES_DISABLED) {
        return;
    }
    initPerformanceVideo();
    injectMonetization();
    ...
    setTimeout(spawnConnectionPulse, 15000);
        ...
        if (page === 'index.html' || page === '') {
            setInterval(spawnPerformanceAd, 25000);
        }
    }
...
/**
 * [+] INITIALIZE CINEMATIC SIDE PODS
 * Fixed containers playing randomized hyper-realistic travel/car videos.
 */
async function initCinematicPods() {
    if (window.OBSIDIAN_PERFORMANCE_PODS_DISABLED) return;

    const pods = ['left', 'right'];
    const queries = ['cars driving highway', 'us road trip cinematic', 'city traffic night aerial', 'mountain road drive'];

    pods.forEach(async (side) => {
        const pod = document.createElement('div');
        pod.className = `performance-cinematic-pod pod-${side} performance-fade-up`;
        ...
        label.innerText = side === 'left' ? 'Global Network' : 'Edge Performance';

        pod.appendChild(video);
        pod.appendChild(label);
        document.body.appendChild(pod);

        try {
            const query = queries[Math.floor(Math.random() * queries.length)];
            const resp = await fetch(getApiUrl(`/api/performance/video?query=${encodeURIComponent(query)}`));
            const data = await resp.json();
            if (data.success) {
            ...
            const fallbacks = [
                "https://player.vimeo.com/external/371728562.hd.mp4?s=447702f23cf5354900cb3e23630f9a56763a14e9&profile_id=175",
                "https://assets.mixkit.co/videos/preview/mixkit-aerial-view-of-city-traffic-at-night-11-large.mp4"
            ];
            const source = document.createElement('source');
            source.src = fallbacks[side === 'left' ? 0 : 1];
            source.type = 'video/mp4';
            video.appendChild(source);
            video.load();
        }

        setTimeout(() => pod.classList.add('visible'), 500);
    });
}
function injectInfomercials() {
    if (window.OBSIDIAN_PERFORMANCE_PULSES_DISABLED) return;

    // Check if already injected or present
    if (document.querySelector('.obsidian-infomercial-section')) return;

    const section = document.createElement('section');
    section.className = 'obsidian-infomercial-section performance-fade-up';
    ...
    const themedClips = [
        ...
        { label: "Quantum Network Pulse", src: "https://assets.mixkit.co/videos/preview/mixkit-futuristic-technology-background-loop-91-large.mp4" },
