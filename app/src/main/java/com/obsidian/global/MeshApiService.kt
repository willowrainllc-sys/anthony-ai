...
data class WholesaleTelemetry(
    @SerializedName("total_gb_routed_24h") val totalGbRouted24h: Double,
    @SerializedName("active_contracts_count") val activeContractsCount: Int,
    @SerializedName("port_matrix_status") val portMatrixStatus: String,
    @SerializedName("current_yield_usd") val currentYieldUsd: Double
)
...
data class RevenueVitalsResponse(
    ...
    @SerializedName("square_real_settled_usd") val squareRealSettledUsd: Double = 0.0,
    @SerializedName("square_status") val squareStatus: String = "LOCKED"
)

data class IptvSignalsResponse(
    val status: String,
    val signals: List<StreetCamera>
)
...
interface MeshApiService {
    ...
    @POST("/api/mesh/sync-time-history")
    suspend fun syncTimeHistory(
        @Query("node_id") nodeId: String,
    ...
    @GET("/api/iptv/signals")
    suspend fun getIptvSignals(): IptvSignalsResponse

    // 🔱 Sovereign Pay (Cash App Clone)
    @GET("/api/pay/balance")
    suspend fun getPayBalance(@Query("cashtag") cashtag: String): Map<String, Double>

    @POST("/api/pay/transfer")
    suspend fun payTransfer(@Body payload: Map<String, Any?>): Map<String, Any?>

    @POST("/api/pay/register")
    suspend fun payRegister(@Body payload: Map<String, String>): Map<String, String>
    ...
    companion object {
        ...
        fun createWithClient(ip: String, customClient: OkHttpClient): MeshApiService {
            // Ensure the custom client also has the required Obsidian Key
            val securedClient = customClient.newBuilder()
                .addInterceptor { chain ->
                    val request = chain.request().newBuilder()
                        .addHeader("x-api-key", OBSIDIAN_API_KEY)
                        .build()
